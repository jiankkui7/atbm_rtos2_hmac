...
//	pTimer->TimerHander = MMPF_OS_CREATE_TMR_INT_ERR;
	...
	return 0;
#endif
	//pTimer->type=1;          //no msg ,mask 1
	if(pCallback !=NULL){
		pTimer->func=pCallback;
	}else{
		return -1;
	}	
	...
	pTimer->handle = timer_setup(portMAX_DELAY, 0, pTimer->func, pTimer->arg); //xTimerCreate
	if(pTimer->handle == 0) {
		printf("[%s] create the timer fail \n",__func__);
		return -1;
	}
...
#endif
	int ret = 0;
	if(timer_pending_api(pTimer->handle)) {// can't reurn -1
		 ;
		 //printf("[%s] time is pending\n", __func__);	 
	 }
//	printf("timer_func:%x\n",pTimer->func);
	ret = xTimerChangePeriod(pTimer->handle, Interval, 0);//no need xTimerStart again
	if(ret != pdPASS) { 
		 printf("[%s] change the time fail\n", __func__);
		 return -1;
	}
	return 0;
...
PUBLIC int atbm_CancelTimer(OS_TIMER *pTimer)
{
	...
	return 0;
#endif
	del_timer_api(pTimer->handle); //xTimerStop
	return 0;
}
...
#endif
	signed int ret = 0;
	ret = timer_free(pTimer->handle);
	if (ret == -1) {
		printf("[%s] destroy the timer fail \n",__func__);
		return -1;
	}
	return 0;
}
...
{
	HASH_LOCK_INIT();                                   //?????
	atbm_memset(atbm_timer_hash,0,sizeof(atbm_timer_hash));
}
