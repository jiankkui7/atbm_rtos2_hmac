...
int atbm_akwifi_setup_sdio(void)
{
	...
	if(i == 0){
		...
		rt_kprintf("find sdio index in dtb fail\n");
		return -1;
	}

	rt_kprintf("fixed sdio bus mode =%d,interface =%d\n",bus_width,cif);
...
int atbm_akwifi_netif_init(struct atbm_net_device *dev)
{
	...
	if(atbm_get_wifimode(priv) == WIFI_MODE_AP){
		dhcps_start(ipaddr);
	}else{
		dhcp_start(p_netif);
