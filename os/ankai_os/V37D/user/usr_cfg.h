...
//
//Support get PMK in host
//
#define	CONFIG_HOST_PMK	//add by rongqi xu
...
#define sdio_claim_host(x) woal_sdio_host_claim()
#define sdio_release_host(x) woal_sdio_host_unclaim()
...
extern struct bss_descriptor dest_bss;
...
#define xrf_htons(x)	((uint16_t)((((x) & 0xff) << 8) | (((x)& 0xff00) >> 8)))
...
#define ARRAY_SIZE2(x,y)	(sizeof(x)/sizeof(y))
