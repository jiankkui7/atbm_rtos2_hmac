						...
						int dev_pw_id)
{
	...
	if (wfd_ie && p2p->inv_role == P2P_INVITE_ROLE_ACTIVE_GO) {
		...
		for (i = 0; i < p2p->num_groups; i++) {
			...
			struct wpabuf *ie;
			if (atbm_memcmp(p2p_group_get_interface_addr(g),
				      p2p->inv_bssid, ATBM_ETH_ALEN) != 0)
				continue;
...
static struct wpabuf * p2p_build_invitation_resp(struct p2p_data *p2p,
						 struct p2p_device *peer,
						 atbm_uint8 dialog_token, atbm_uint8 status,
				...
				atbm_uint8 *data, atbm_size_t len, int rx_freq)
{
	...
	atbm_uint8 status = P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE;
	int freq;
	int go = 0;
	atbm_uint8 group_bssid[ATBM_ETH_ALEN], *bssid;
	int op_freq = 0;
	atbm_uint8 reg_class = 0, channel = 0;
	struct p2p_channels all_channels, intersection, *channels = NULL;
	int persistent;

	atbm_memset(group_bssid, 0, sizeof(group_bssid));
	...
	if (p2p->cfg->invitation_process) {
		status = p2p->cfg->invitation_process(
			...
			msg.group_id + ATBM_ETH_ALEN, msg.group_id_len - ATBM_ETH_ALEN,
...
fail:
	if (go && status == P2P_SC_SUCCESS && !atbm_is_zero_ether_addr(group_bssid))
		bssid = group_bssid;
	else
		bssid = NULL;
	 ...
	 */
	atbm_memcpy(p2p->inv_sa, sa, ATBM_ETH_ALEN);
	if (msg.group_bssid) {
		atbm_memcpy(p2p->inv_group_bssid, msg.group_bssid, ATBM_ETH_ALEN);
		p2p->inv_group_bssid_ptr = p2p->inv_group_bssid;
	} else
		p2p->inv_group_bssid_ptr = NULL;
	if (msg.group_id) {
		if (msg.group_id_len - ATBM_ETH_ALEN <= SSID_MAX_LEN) {
			atbm_memcpy(p2p->inv_ssid, msg.group_id + ATBM_ETH_ALEN,
				  msg.group_id_len - ATBM_ETH_ALEN);
			p2p->inv_ssid_len = msg.group_id_len - ATBM_ETH_ALEN;
		}
		atbm_memcpy(p2p->inv_go_dev_addr, msg.group_id, ATBM_ETH_ALEN);
	} else {
		p2p->inv_ssid_len = 0;
		atbm_memset(p2p->inv_go_dev_addr, 0, ATBM_ETH_ALEN);
	}
	       ...
	       int persistent_group, unsigned int pref_freq, int dev_pw_id)
{
	...
	if (go_dev_addr) {
		p2p_dbg(p2p, "Invitation for GO Device Address " MACSTR,
			MAC2STR(go_dev_addr));
		atbm_memcpy(p2p->invite_go_dev_addr_buf, go_dev_addr, ATBM_ETH_ALEN);
	...
	if (bssid)
		atbm_memcpy(p2p->inv_bssid, bssid, ATBM_ETH_ALEN);
	atbm_memcpy(p2p->inv_ssid, ssid, ssid_len);
