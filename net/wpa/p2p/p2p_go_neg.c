...
static int p2p_go_det(atbm_uint8 own_intent, atbm_uint8 peer_value)
{
	atbm_uint8 peer_intent = peer_value >> 1;
			    ...
			    const atbm_uint8 *channel_list, atbm_size_t channel_list_len)
{
	...
	if (end - pos < 3)
		return -1;
	atbm_memcpy(dev->country, pos, 3);
	wpa_hexdump_ascii(MSG_DEBUG, "P2P: Peer country", pos, 3);
	if (pos[2] != 0x04 && atbm_memcmp(pos, p2p->cfg->country, 2) != 0) {
	...
	while (end - pos > 2) {
		...
		atbm_memcpy(cl->channel, pos, cl->channels);
		pos += channels;
		ch->reg_classes++;
			  ...
			  struct p2p_channels *intersection)
{
	...
	int freq;
	atbm_uint8 op_reg_class, op_channel;
	unsigned int i;
			    ...
			    atbm_uint8 *data, atbm_size_t len, int rx_freq)
{
	...
	struct wpabuf *resp;
	struct p2p_message msg;
	atbm_uint8 status = P2P_SC_FAIL_INVALID_PARAMS;
	int tie_breaker = 0;
	int freq;
	...
	if (atbm_memcmp(msg.p2p_device_addr, sa, ATBM_ETH_ALEN) != 0) {
	...
	if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
		...
		if (dev->go_neg_req_sent &&
		    atbm_memcmp(sa, p2p->cfg->dev_addr, ATBM_ETH_ALEN) > 0) {
		...
		atbm_memcpy(dev->intended_addr, msg.intended_addr, ATBM_ETH_ALEN);
		p2p->go_neg_peer = dev;
		atbmwifi_eloop_cancel_timeout(p2p_go_neg_wait_timeout, p2p, NULL);
...
fail:
	...
	if (status == P2P_SC_SUCCESS) {
		...
		if (atbm_memcmp(sa, p2p->cfg->dev_addr, ATBM_ETH_ALEN) < 0) {
			/*
...
static struct wpabuf * p2p_build_go_neg_conf(struct p2p_data *p2p,
					     struct p2p_device *peer,
					     atbm_uint8 dialog_token, atbm_uint8 status,
					     const atbm_uint8 *resp_chan, int go)
{
	struct wpabuf *buf;
	atbm_uint8 *len;
	struct p2p_channels res;
			     ...
			     atbm_uint8 *data, atbm_size_t len, int rx_freq)
{
	...
	if (*msg.status) {
		...
		if (*msg.status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) {
			...
			dev->flags |= P2P_DEV_NOT_YET_READY;
			atbmwifi_eloop_cancel_timeout(p2p_go_neg_wait_timeout, p2p,
					     NULL);
			atbmwifi_eloop_register_timeout(120, 0, p2p_go_neg_wait_timeout,
					       p2p, NULL);
	...
	if (!go && msg.group_id) {
		...
		p2p->ssid_len = msg.group_id_len - ATBM_ETH_ALEN;
		atbm_memcpy(p2p->ssid, msg.group_id + ATBM_ETH_ALEN, p2p->ssid_len);
	} else if (!go) {
