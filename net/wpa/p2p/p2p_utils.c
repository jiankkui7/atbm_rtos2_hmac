...
int p2p_random(char *buf, atbm_size_t len)
{
	atbm_uint8 val;
	atbm_size_t i;
	atbm_uint8 letters = 'Z' - 'A' + 1;
	atbm_uint8 numbers = 10;

	if (atbmwifi_os_get_random((unsigned char *) buf, len))
		return -1;
...
{
	if (ieee80211_freq_to_chan(freq, channel) == ATBM_NUM_HOSTAPD_MODES){
		return -1;
	}
				...
				const struct p2p_channels *b)
{
	...
	for (j = 0; j < b->reg_classes; j++) {
		...
		if (i == res->reg_classes) {
			if (res->reg_classes == P2P_MAX_REG_CLASSES)
				return;
			atbm_memcpy(&res->reg_class[res->reg_classes++],
...
static atbm_uint8 p2p_channel_pick_random(const atbm_uint8 *channels, unsigned int num_channels)
{
	unsigned int r;
