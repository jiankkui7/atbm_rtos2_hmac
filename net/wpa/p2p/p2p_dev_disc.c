...
int p2p_send_dev_disc_req(struct p2p_data *p2p, struct p2p_device *dev)
{
	...
	atbm_memcpy(p2p->pending_client_disc_addr, dev->info.p2p_device_addr,
		  ATBM_ETH_ALEN);
...
static struct wpabuf * p2p_build_dev_disc_resp(atbm_uint8 dialog_token, atbm_uint8 status)
{
	struct wpabuf *buf;
	atbm_uint8 *len;
...
static void p2p_send_dev_disc_resp(struct p2p_data *p2p, atbm_uint8 dialog_token,
				   const atbm_uint8 *addr, int freq, atbm_uint8 status)
{
	struct wpabuf *resp;
			      ...
			      atbm_uint8 *data, atbm_size_t len, int rx_freq)
{
	...
	for (g = 0; g < p2p->num_groups; g++) {
		if (p2p_group_go_discover(p2p->groups[g], msg.device_id, sa,
			...
			atbm_memcpy(p2p->pending_dev_disc_addr, sa, ATBM_ETH_ALEN);
			       ...
			       atbm_uint8 *data, atbm_size_t len)
{
	...
	if (go == NULL ||
	    atbm_memcmp(sa, go->info.p2p_device_addr, ATBM_ETH_ALEN) != 0) {
	...
	if (p2p->go_neg_peer == NULL ||
	    atbm_memcmp(p2p->pending_client_disc_addr,
		      p2p->go_neg_peer->info.p2p_device_addr, ATBM_ETH_ALEN) != 0 ||
	    atbm_memcmp(p2p->go_neg_peer->member_in_go_dev,
		      go->info.p2p_device_addr, ATBM_ETH_ALEN) != 0) {
