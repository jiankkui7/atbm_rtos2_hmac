...
static void p2ps_add_pd_req_attrs(struct p2p_data *p2p, struct p2p_device *dev,
				  struct wpabuf *buf, atbm_uint16 config_methods)
{
	...
	if (prov->status >= 0)
		p2p_buf_add_status(buf, (atbm_uint8) prov->status);
	else
	...
	if (!follow_on_req_fail) {
		...
		if ((shared_group && !atbm_is_zero_ether_addr(intended_addr)) ||
		    (prov->conncap & (P2PS_SETUP_GROUP_OWNER | P2PS_SETUP_NEW)))
	...
	p2p_buf_add_feature_capability(buf, sizeof(fcap), (const atbm_uint8 *) &fcap);
	...
	if (shared_group) {
		...
		if ((prov->conncap == P2PS_SETUP_NONE ||
		    ...
		    !atbm_is_zero_ether_addr(intended_addr))
			p2p_buf_add_intended_addr(buf, intended_addr);
	}
}
					       ...
					       int join)
{
	...
	atbm_uint8 *len;
	atbm_size_t extra = 0;
	atbm_uint8 dialog_token = dev->dialog_token;
	atbm_uint16 config_methods = dev->req_config_methods;
...
static struct wpabuf * p2p_build_prov_disc_resp(struct p2p_data *p2p,
						...
						atbm_size_t persist_ssid_len,
						const atbm_uint8 *fcap,
						atbm_uint16 fcap_len)
{
	struct wpabuf *buf;
	atbm_size_t extra = 0;
	int persist = 0;
	...
	if (p2p->p2ps_prov && p2p->p2ps_prov->adv_id == adv_id) {
		atbm_uint8 *len = p2p_buf_add_ie_hdr(buf);
		struct p2ps_provision *prov = p2p->p2ps_prov;
		atbm_uint8 group_capab;
		atbm_uint8 conncap = 0;
		...
		if (persist_ssid && p2p->cfg->get_persistent_group && dev &&
			...
			if (persist) {
				...
				if (!atbm_is_zero_ether_addr(intended_addr))
					p2p_buf_add_intended_addr(
						buf, intended_addr);
			}
		}
...
static int p2ps_setup_p2ps_prov(struct p2p_data *p2p, atbm_uint32 adv_id,
				atbm_uint32 session_id, atbm_uint16 method,
				const atbm_uint8 *session_mac, const atbm_uint8 *adv_mac)
{
	struct p2ps_provision *tmp;

	if (!p2p->p2ps_prov) {
		p2p->p2ps_prov = (struct p2ps_provision *)atbm_kzalloc(sizeof(struct p2ps_provision) + 1, GFP_KERNEL);
		if (!p2p->p2ps_prov)
			return -1;
	} else {
		atbm_memset(p2p->p2ps_prov, 0, sizeof(struct p2ps_provision) + 1);
	}
	...
	tmp->method = method;
	atbm_memcpy(tmp->session_mac, session_mac, ATBM_ETH_ALEN);
	atbm_memcpy(tmp->adv_mac, adv_mac, ATBM_ETH_ALEN);
	tmp->info[0] = '\0';
...
static atbm_uint8 p2ps_own_preferred_cpt(const atbm_uint8 *cpt_priority, atbm_uint8 req_cpt_mask)
{
	int i;
...
{
	atbm_uint8 group_id = 0;
	atbm_uint8 intended_addr = 0;
	atbm_uint8 operating_channel = 0;
	atbm_uint8 channel_list = 0;
	atbm_uint8 config_timeout = 0;
	atbm_uint8 listen_channel = 0;
...
do { \
	...
	if (msg->persistent_dev) {
		channel_list = 1;
		config_timeout = 1;
		if (atbm_memcmp(msg->persistent_dev, addr, ATBM_ETH_ALEN) == 0) {
			intended_addr = 1;
			operating_channel = 1;
		}
	}
			       ...
			       atbm_uint8 *data, atbm_size_t len, int rx_freq)
{
	...
	atbm_uint8 conncap = P2PS_SETUP_NEW;
	atbm_uint8 auto_accept = 0;
	atbm_uint32 session_id = 0;
	atbm_uint8 session_mac[ATBM_ETH_ALEN] = { 0, 0, 0, 0, 0, 0 };
	atbm_uint8 adv_mac[ATBM_ETH_ALEN] = { 0, 0, 0, 0, 0, 0 };
	const atbm_uint8 *group_mac;
	int passwd_id = DEV_PW_DEFAULT;
	atbm_uint16 config_methods;
	atbm_uint16 allowed_config_methods = WPS_CONFIG_DISPLAY | WPS_CONFIG_KEYPAD;
	struct p2ps_feature_capab resp_fcap = { 0, 0 };
	struct p2ps_feature_capab *req_fcap = NULL;
	atbm_uint8 remote_conncap;
	atbm_uint16 method;
	...
	if (!msg.adv_id) {
		 ...
		 * will be sent.
		 */
		adv_id = ATBM_WPA_GET_LE32(msg.adv_id);
		...
		atbm_memcpy(session_mac, msg.session_mac, ATBM_ETH_ALEN);
		atbm_memcpy(adv_mac, msg.adv_mac, ATBM_ETH_ALEN);

		session_id = ATBM_WPA_GET_LE32(msg.session_id);
	...
	if (!msg.status) {
		...
		if (atbm_memcmp(p2p->cfg->dev_addr, msg.adv_mac, ATBM_ETH_ALEN)) {
			p2p_dbg(p2p,
	...
	if (p2p->p2ps_prov->adv_id != adv_id ||
	    atbm_memcmp(p2p->p2ps_prov->adv_mac, msg.adv_mac, ATBM_ETH_ALEN)) {
		p2p_dbg(p2p,
	...
	if (p2p->p2ps_prov->session_id != session_id ||
	    atbm_memcmp(p2p->p2ps_prov->session_mac, msg.session_mac, ATBM_ETH_ALEN)) {
...
out:
	...
	if (!p2p->cfg->p2ps_prov_complete) {
		...
		p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, P2P_SC_SUCCESS,
					     ...
					     sizeof(resp_fcap), freq,
					     msg.group_id ?
					     msg.group_id + ATBM_ETH_ALEN : NULL,
					     msg.group_id ?
					     msg.group_id_len - ATBM_ETH_ALEN : 0);
		...
		atbm_size_t buf_len = msg.session_info_len;
		char *buf = (char *)atbm_kmalloc(2 * buf_len + 1, GFP_KERNEL);
		...
		if (buf) {
			...
			p2p->cfg->p2ps_prov_complete(
				...
				msg.persistent_ssid, msg.persistent_ssid_len,
				0, 1, buf,
				(const atbm_uint8 *) &resp_fcap, sizeof(resp_fcap),
				freq, NULL, 0);
	...
	if (reject == P2P_SC_SUCCESS) {
		...
		if (msg.intended_addr)
			atbm_memcpy(dev->interface_addr, msg.intended_addr,
				  ATBM_ETH_ALEN);
	}
	p2p_parse_free(&msg);
}
...
static int p2p_validate_p2ps_pd_resp(struct p2p_data *p2p,
				     struct p2p_message *msg)
{
	atbm_uint8 conn_cap_go = 0;
	atbm_uint8 conn_cap_cli = 0;
...
#define P2PS_PD_RESP_CHECK(_val, _attr) \
	...
	if (atbm_memcmp(p2p->p2ps_prov->session_mac, msg->session_mac,
		      ATBM_ETH_ALEN)) {
		p2p_dbg(p2p,
	...
	if (atbm_memcmp(p2p->p2ps_prov->adv_mac, msg->adv_mac, ATBM_ETH_ALEN) != 0) {
		p2p_dbg(p2p,
				...
				atbm_uint8 *data, atbm_size_t len)
{
	struct p2p_message msg;
	struct p2p_device *dev;
	atbm_uint16 report_config_methods = 0, req_config_methods;
	atbm_uint8 status = P2P_SC_SUCCESS;
	atbm_uint32 adv_id = 0;
	atbm_uint8 conncap = P2PS_SETUP_NEW;
	...
	if (msg.adv_mac)
		...
		atbm_memset(adv_mac, 0, ATBM_ETH_ALEN);
	...
	if (p2p->pending_action_state == P2P_PENDING_PD) {
		atbm_memset(p2p->pending_pd_devaddr, 0, ATBM_ETH_ALEN);
	...
	if (p2p->user_initiated_pd &&
	    atbm_memcmp(p2p->pending_pd_devaddr, sa, ATBM_ETH_ALEN) == 0)
	...
	if ((status == P2P_SC_SUCCESS || status == P2P_SC_SUCCESS_DEFERRED) &&
		...
		if (p2p->cfg->p2ps_prov_complete) {
			...
			p2p->cfg->p2ps_prov_complete(
				...
				msg.group_id ? msg.group_id + ATBM_ETH_ALEN : NULL,
				msg.group_id ? msg.group_id_len - ATBM_ETH_ALEN : 0);
		}
		p2ps_prov_free(p2p);
	...
	if (status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) {
		...
		if (msg.session_info && msg.session_info_len) {
			atbm_size_t info_len = msg.session_info_len;
			char *deferred_sess_resp = (char *)atbm_kmalloc(2 * info_len + 1, GFP_KERNEL);
	...
	if (msg.intended_addr)
		atbm_memcpy(dev->interface_addr, msg.intended_addr, ATBM_ETH_ALEN);
			   ...
			   int join, int force_freq)
{
	...
	atbm_memcpy(p2p->pending_pd_devaddr, dev->info.p2p_device_addr, ATBM_ETH_ALEN);
...
int p2p_prov_disc_req(struct p2p_data *p2p, const atbm_uint8 *peer_addr,
		      ...
		      atbm_uint16 config_methods, int join, int force_freq,
...
void p2p_reset_pending_pd(struct p2p_data *p2p)
{
	...
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		if (atbm_memcmp(p2p->pending_pd_devaddr,
			      dev->info.p2p_device_addr, ATBM_ETH_ALEN))
			continue;
