...
static int wfd_wsd_supported(struct wpabuf *wfd)
{
	...
	while (end - pos >= 3) {
		...
		if (subelem == WFD_SUBELEM_DEVICE_INFO && len >= 6) {
			atbm_uint16 info = ATBM_WPA_GET_BE16(pos);
			return !!(info & 0x0040);
		}
					 ...
					 struct p2p_device *dev)
{
	...
	for (q = p2p->sd_queries; q; q = q->next) {
		...
		if (!q->for_all_peers &&
		    os_memcmp(q->peer, dev->info.p2p_device_addr, ATBM_ETH_ALEN) ==
		    0)
			goto found;
	}
...
static struct wpabuf * p2p_build_sd_query(atbm_uint16 update_indic,
					  struct wpabuf *tlvs)
{
	return NULL;
#if 0
...
static void p2p_send_gas_comeback_req(struct p2p_data *p2p, const atbm_uint8 *dst,
				      atbm_uint8 dialog_token, int freq)
{
	return;
...
static struct wpabuf * p2p_build_sd_response(atbm_uint8 dialog_token, atbm_uint16 status_code,
					     atbm_uint16 comeback_delay,
					     atbm_uint16 update_indic,
...
static struct wpabuf * p2p_build_gas_comeback_resp(atbm_uint8 dialog_token,
						   atbm_uint16 status_code,
						   atbm_uint16 update_indic,
						   const atbm_uint8 *data, atbm_size_t len,
						   atbm_uint8 frag_id, atbm_uint8 more,
						   atbm_uint16 total_len)
{
	return NULL;
#if 0
	struct wpabuf *buf;
...
{
#if 0
	const atbm_uint8 *pos = data;
	const atbm_uint8 *end = data + len;
	const atbm_uint8 *next;
	...
	if (end - pos < 4)
		return;
	if (ATBM_WPA_GET_LE16(pos) != ANQP_VENDOR_SPECIFIC) {
		p2p_dbg(p2p, "Unsupported ANQP Info ID %u", ATBM_WPA_GET_LE16(pos));
		return;
	}
	pos += 2;
	...
	if (ATBM_WPA_GET_BE32(pos) != P2P_IE_VENDOR_TYPE) {
		p2p_dbg(p2p, "Unsupported ANQP vendor OUI-type %08x",
			ATBM_WPA_GET_BE32(pos));
		return;
	}
	pos += 4;
	...
	if (end - pos < 2)
		return;
	update_indic = ATBM_WPA_GET_LE16(pos);
		     ...
		     atbm_uint8 dialog_token, const struct wpabuf *resp_tlvs)
{
	...
	if (wpabuf_len(resp_tlvs) > max_len) {
		...
		os_memcpy(p2p->sd_resp_addr, dst, ATBM_ETH_ALEN);
			     ...
			     const atbm_uint8 *data, atbm_size_t len, int rx_freq)
{
	...
	if (p2p->state != P2P_SD_DURING_FIND || p2p->sd_peer == NULL ||
	    os_memcmp(sa, p2p->sd_peer->info.p2p_device_addr, ATBM_ETH_ALEN) != 0) {
	...
	status_code = ATBM_WPA_GET_LE16(pos);
	pos += 2;
	comeback_delay = ATBM_WPA_GET_LE16(pos);
	pos += 2;
			     ...
			     const atbm_uint8 *data, atbm_size_t len, int rx_freq)
{
	...
	struct wpabuf *resp;
	atbm_uint8 dialog_token;
	atbm_size_t frag_len, max_len;
	int more = 0;
	unsigned int wait_time = 200;

	p2p_hexdump(MSG_DEBUG, "P2P: RX GAS Comeback Request", data, len);
	if (len < 1)
		return;
	dialog_token = *data;
	...
	if (os_memcmp(sa, p2p->sd_resp_addr, ATBM_ETH_ALEN) != 0) {
			      ...
			      const atbm_uint8 *data, atbm_size_t len, int rx_freq)
{
	...
	atbm_uint16 status_code;
	atbm_uint8 frag_id;
	atbm_uint8 more_frags;
	atbm_uint16 comeback_delay;
	atbm_uint16 slen;

	p2p_hexdump(MSG_DEBUG, "P2P: RX GAS Comeback Response", data, len);
	...
	if (end - pos < 2)
		return;
	p2p->sd_rx_update_indic = ATBM_WPA_GET_LE16(pos);
		      ...
		      const struct wpabuf *tlvs)
{
	...
	q = (struct p2p_sd_query *)atbm_kzalloc(sizeof(*q), GFP_KERNEL);
	if (q == NULL)
		return NULL;

	if (dst)
		os_memcpy(q->peer, dst, ATBM_ETH_ALEN);
	else
		q->for_all_peers = 1;
