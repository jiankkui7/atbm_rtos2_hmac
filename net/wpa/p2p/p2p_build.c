...
void p2p_buf_add_action_hdr(struct wpabuf *buf, atbm_uint8 subtype, atbm_uint8 dialog_token)
{
	...
	p2p_printf(MSG_DEBUG, "P2P: * Dialog Token: %d", dialog_token);
}
...
atbm_uint8 * p2p_buf_add_ie_hdr(struct wpabuf *buf)
{
	...
	/* P2P IE header */
	wpabuf_put_u8(buf, ATBM_WLAN_EID_VENDOR_SPECIFIC);
	len = wpabuf_put(buf, 1); /* IE length to be filled */
	wpabuf_put_be32(buf, P2P_IE_VENDOR_TYPE);
	p2p_printf(MSG_DEBUG, "P2P: * P2P IE header");
	return len;
}
...
void p2p_buf_update_ie_hdr(struct wpabuf *buf, atbm_uint8 *len)
{
	...
	*len = (atbm_uint8 *) wpabuf_put(buf, 0) - len - 1;
}
...
void p2p_buf_add_capability(struct wpabuf *buf, atbm_uint8 dev_capab, atbm_uint8 group_capab)
{
	...
	p2p_printf(MSG_DEBUG, "P2P: * Capability dev=%02x group=%02x",
...
void p2p_buf_add_go_intent(struct wpabuf *buf, atbm_uint8 go_intent)
{
	...
	p2p_printf(MSG_DEBUG, "P2P: * GO Intent: Intent %u Tie breaker %u",
				...
				atbm_uint8 reg_class, atbm_uint8 channel)
{
	...
	p2p_printf(MSG_DEBUG, "P2P: * Listen Channel: Regulatory Class %u "
				   ...
				   atbm_uint8 reg_class, atbm_uint8 channel)
{
	...
	wpabuf_put_u8(buf, reg_class); /* Regulatory Class */
	wpabuf_put_u8(buf, channel); /* Channel Number */
	p2p_printf(MSG_DEBUG, "P2P: * Operating Channel: Regulatory Class %u "
		   "Channel %u", reg_class, channel);
}


void p2p_buf_add_pref_channel_list(struct wpabuf *buf,
				   const atbm_uint32 *preferred_freq_list,
				   unsigned int size)
{
	...
	wpabuf_put_u8(buf, ATBM_WLAN_EID_VENDOR_SPECIFIC);
	wpabuf_put_u8(buf, 4 + count * sizeof(atbm_uint16));
	wpabuf_put_be24(buf, OUI_QCA);
	wpabuf_put_u8(buf, QCA_VENDOR_ELEM_P2P_PREF_CHAN_LIST);
	for (i = 0; i < size; i++) {
		if (p2p_freq_to_channel(preferred_freq_list[i], &op_class,
					&op_channel) < 0) {
			p2p_printf(MSG_DEBUG, "Unsupported frequency %u MHz",
			      ...
			      struct p2p_channels *chan)
{
	...
	ATBM_WPA_PUT_LE16(len, (atbm_uint8 *) wpabuf_put(buf, 0) - len - 2);
	p2p_hexdump(MSG_DEBUG, "P2P: * Channel List",
		    len + 2, (atbm_uint8 *) wpabuf_put(buf, 0) - len - 2);
}
...
void p2p_buf_add_status(struct wpabuf *buf, atbm_uint8 status)
{
	...
	p2p_printf(MSG_DEBUG, "P2P: * Status: %d", status);
}
			     ...
			     struct p2p_device *peer)
{
	...
	/* P2P Device address */
	wpabuf_put_data(buf, p2p->cfg->dev_addr, ATBM_ETH_ALEN);
	...
	/* Update attribute length */
	ATBM_WPA_PUT_LE16(len, (atbm_uint8 *) wpabuf_put(buf, 0) - len - 2);
	p2p_printf(MSG_DEBUG, "P2P: * Device Info");
}
...
void p2p_buf_add_device_id(struct wpabuf *buf, const atbm_uint8 *dev_addr)
{
	...
	wpabuf_put_le16(buf, ATBM_ETH_ALEN);
	wpabuf_put_data(buf, dev_addr, ATBM_ETH_ALEN);
	p2p_printf(MSG_DEBUG, "P2P: * Device ID: " MACSTR, MAC2STR(dev_addr));
}
				...
				atbm_uint8 client_timeout)
{
	...
	p2p_printf(MSG_DEBUG, "P2P: * Configuration Timeout: GO %d (*10ms)  "
...
void p2p_buf_add_intended_addr(struct wpabuf *buf, const atbm_uint8 *interface_addr)
{
	...
	wpabuf_put_le16(buf, ATBM_ETH_ALEN);
	wpabuf_put_data(buf, interface_addr, ATBM_ETH_ALEN);
	p2p_printf(MSG_DEBUG, "P2P: * Intended P2P Interface Address " MACSTR,
...
void p2p_buf_add_group_bssid(struct wpabuf *buf, const atbm_uint8 *bssid)
{
	...
	wpabuf_put_le16(buf, ATBM_ETH_ALEN);
	wpabuf_put_data(buf, bssid, ATBM_ETH_ALEN);
			  ...
			  const atbm_uint8 *ssid, atbm_size_t ssid_len)
{
	...
	wpabuf_put_le16(buf, ATBM_ETH_ALEN + ssid_len);
	wpabuf_put_data(buf, dev_addr, ATBM_ETH_ALEN);
	wpabuf_put_data(buf, ssid, ssid_len);
	p2p_printf(MSG_DEBUG, "P2P: * P2P Group ID " MACSTR,
		   MAC2STR(dev_addr));
	p2p_hexdump_ascii(MSG_DEBUG, "P2P: P2P Group ID SSID", ssid, ssid_len);
}
...
void p2p_buf_add_invitation_flags(struct wpabuf *buf, atbm_uint8 flags)
{
	...
	p2p_printf(MSG_DEBUG, "P2P: * Invitation Flags: bitmap 0x%x", flags);
}
				   ...
				   atbm_uint16 interval)
{
	...
	p2p_printf(MSG_DEBUG, "P2P: * Extended Listen Timing (period %u msec  "
...
void p2p_buf_add_p2p_interface(struct wpabuf *buf, struct p2p_data *p2p)
{
	...
	wpabuf_put_le16(buf, ATBM_ETH_ALEN + 1 + ATBM_ETH_ALEN);
	/* P2P Device address */
	wpabuf_put_data(buf, p2p->cfg->dev_addr, ATBM_ETH_ALEN);
	/*
				    ...
				    enum p2p_role_indication role)
{
	...
	wpabuf_put_u8(buf, channel); /* Channel Number */
	wpabuf_put_u8(buf, (atbm_uint8) role); /* Role indication */
	p2p_printf(MSG_DEBUG, "P2P: * OOB GO Negotiation Channel: Operating "
...
void p2p_buf_add_service_hash(struct wpabuf *buf, struct p2p_data *p2p)
{
	...
	p2p_hexdump(MSG_DEBUG, "P2P: * Service Hash",
...
void p2p_buf_add_session_info(struct wpabuf *buf, const char *info)
{
	...
	wpabuf_put_le16(buf, (atbm_uint16) info_len);

	if (info) {
		wpabuf_put_data(buf, info, info_len);
		p2p_printf(MSG_DEBUG, "P2P: * Session Info Data (%s)", info);
	}
}
...
void p2p_buf_add_advertisement_id(struct wpabuf *buf, atbm_uint32 id, const atbm_uint8 *mac)
{
	...
	wpabuf_put_le16(buf, (atbm_uint16) (sizeof(atbm_uint32) + ATBM_ETH_ALEN));
	wpabuf_put_le32(buf, id);
	wpabuf_put_data(buf, mac, ATBM_ETH_ALEN);
	p2p_printf(MSG_DEBUG, "P2P: * Advertisement ID (%x) " MACSTR,
		   id, MAC2STR(mac));
}


static int p2ps_wildcard_hash(struct p2p_data *p2p,
			      const atbm_uint8 *hash, atbm_uint8 hash_count)
{
	atbm_uint8 i;
	const atbm_uint8 *test = hash;

	for (i = 0; i < hash_count; i++) {
		if (atbm_memcmp(test, p2p->wild_card_hash, P2PS_HASH_LEN) == 0)
			return 1;
		test += P2PS_HASH_LEN;
	}
...
static int p2p_buf_add_service_info(struct wpabuf *buf, struct p2p_data *p2p,
				    atbm_uint32 adv_id, atbm_uint16 config_methods,
				    const char *svc_name, atbm_uint8 **ie_len, atbm_uint8 **pos,
				    atbm_size_t *total_len, atbm_uint8 *attr_len)
{
	atbm_size_t svc_len;
	atbm_size_t remaining;
	atbm_size_t info_len;

	p2p_dbg(p2p, "Add service info for %s (adv_id=%u)", svc_name, adv_id);
	svc_len = os_strlen(svc_name);
	info_len = sizeof(adv_id) + sizeof(config_methods) + sizeof(atbm_uint8) +
		svc_len;
	...
	if (remaining < sizeof(atbm_uint32) + sizeof(atbm_uint16) + sizeof(atbm_uint8)) {
		/*
		 * Split adv_id, config_methods, and svc_name_len between two
		 * IEs.
		 */
		atbm_size_t front = remaining;
		atbm_size_t back = sizeof(atbm_uint32) + sizeof(atbm_uint16) + sizeof(atbm_uint8) - front;
		atbm_uint8 holder[sizeof(atbm_uint32) + sizeof(atbm_uint16) + sizeof(atbm_uint8)];

		ATBM_WPA_PUT_LE32(holder, adv_id);
		ATBM_WPA_PUT_BE16(&holder[sizeof(atbm_uint32)], config_methods);
		holder[sizeof(atbm_uint32) + sizeof(atbm_uint16)] = svc_len;
		...
		*ie_len = p2p_buf_add_ie_hdr(buf);
		...
		remaining = 255 - 4 - (sizeof(atbm_uint32) + sizeof(atbm_uint16) + sizeof(atbm_uint8)) -
			back;
	} else {
	...
	if (remaining < svc_len) {
		...
		atbm_size_t front = remaining;
		atbm_size_t back = svc_len - front;
	...
	*total_len += info_len;
	ATBM_WPA_PUT_LE16(attr_len, (atbm_uint16) *total_len);
	return 0;
}


void p2p_buf_add_service_instance(struct wpabuf *buf, struct p2p_data *p2p,
				  atbm_uint8 hash_count, const atbm_uint8 *hash,
				  struct p2ps_advertisement *adv_list)
{
	...
	atbm_uint8 *pos, *attr_len, *ie_len = NULL;

	if (!adv_list || !hash || !hash_count)
		return;

	p2p_hexdump(MSG_DEBUG, "P2PS: Probe Request service hash values",
	...
	attr_len = wpabuf_put(tmp_buf, sizeof(atbm_uint16));
	ATBM_WPA_PUT_LE16(attr_len, (atbm_uint16) total_len);
	...
	for (adv = adv_list; adv && total_len <= MAX_SVC_ADV_LEN;
		...
		for (i = 0; i < hash_count; i++) {
			...
			if (atbm_memcmp(test, adv->hash, P2PS_HASH_LEN) == 0 &&
...
void p2p_buf_add_session_id(struct wpabuf *buf, atbm_uint32 id, const atbm_uint8 *mac)
{
	...
	wpabuf_put_le32(buf, id);
	wpabuf_put_data(buf, mac, ATBM_ETH_ALEN);
	p2p_printf(MSG_DEBUG, "P2P: * Session ID Info (%x) " MACSTR,
		   id, MAC2STR(mac));
}


void p2p_buf_add_feature_capability(struct wpabuf *buf, atbm_uint16 len, const atbm_uint8 *mask)
{
	if (!buf || !len || !mask)
		return;
	...
	p2p_printf(MSG_DEBUG, "P2P: * Feature Capability (%d)", len);
}
		     ...
		     int all_attr)
{
	...
	if (pw_id >= 0) {
		...
		p2p_printf(MSG_DEBUG, "P2P: WPS IE Device Password ID: %d",
			   pw_id);
