...
static void p2p_process_presence_req(struct p2p_data *p2p, const atbm_uint8 *da,
				     const atbm_uint8 *sa, const atbm_uint8 *data, atbm_size_t len,
				     int rx_freq);
static void p2p_process_presence_resp(struct p2p_data *p2p, const atbm_uint8 *da,
				      const atbm_uint8 *sa, const atbm_uint8 *data,
				      atbm_size_t len);
...
void p2p_set_intended_addr(struct p2p_data *p2p, const atbm_uint8 *intended_addr)
{
	if (p2p && intended_addr)
		atbm_memcpy(p2p->intended_addr, intended_addr, ATBM_ETH_ALEN);
}


atbm_uint16 p2p_get_provisioning_info(struct p2p_data *p2p, const atbm_uint8 *addr)
{
...
void p2p_clear_provisioning_info(struct p2p_data *p2p, const atbm_uint8 *addr)
{
	struct p2p_device *dev = NULL;
...
{
	usec += 500 * 1000; /*add 500 ms*/
	p2p_info(p2p, "Set timeout (state=%s): %u.%06u sec",
		p2p_state_txt(p2p->state), sec, usec);
	atbmwifi_eloop_cancel_timeout(p2p_state_timeout, p2p, NULL);
	atbmwifi_eloop_register_timeout(sec, usec/1000, p2p_state_timeout, p2p, NULL);
}
...
void p2p_go_neg_failed(struct p2p_data *p2p, int status)
{
	...
	atbmwifi_eloop_cancel_timeout(p2p_go_neg_wait_timeout, p2p, NULL);
	...
	atbm_memset(&res, 0, sizeof(res));
	res.status = status;
	atbm_memcpy(res.peer_device_addr, peer->info.p2p_device_addr, ATBM_ETH_ALEN);
	atbm_memcpy(res.peer_interface_addr, peer->intended_addr, ATBM_ETH_ALEN);
...
static void p2p_listen_in_find(struct p2p_data *p2p, int dev_disc)
{
	...
	if (atbmwifi_os_get_random((atbm_uint8 *) &r, sizeof(r)) < 0)
		r = 0;
 ...
 */
struct p2p_device * p2p_get_device(struct p2p_data *p2p, const atbm_uint8 *addr)
{
	struct p2p_device *dev;
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		if (atbm_memcmp(dev->info.p2p_device_addr, addr, ATBM_ETH_ALEN) == 0)
			return dev;
	}
	return NULL;
}
					     ...
					     const atbm_uint8 *addr)
{
	struct p2p_device *dev;
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		if (atbm_memcmp(dev->interface_addr, addr, ATBM_ETH_ALEN) == 0)
			return dev;
	}
	return NULL;
}
					     ...
					     const atbm_uint8 *addr)
{
	...
	if (count + 1 > p2p->cfg->max_peers && oldest) {
		p2p_info(p2p, "Remove oldest peer entry to make room for a new peer");
		dl_list_del(&oldest->list);
		p2p_device_free(p2p, oldest);
	}

	dev = (struct p2p_device *)atbm_kzalloc(sizeof(*dev), GFP_KERNEL);
	if (dev == NULL)
		return NULL;
	dl_list_add(&p2p->devices, &dev->list);
	atbm_memcpy(dev->info.p2p_device_addr, addr, ATBM_ETH_ALEN);
				 ...
				 struct p2p_client_info *cli)
{
	...
	atbm_memcpy(dev->info.pri_dev_type, cli->pri_dev_type, 8);
	dev->info.wps_sec_dev_type_list_len = 8 * cli->num_sec_dev_types;
	atbm_memcpy(dev->info.wps_sec_dev_type_list, cli->sec_dev_types,
		  dev->info.wps_sec_dev_type_list_len);
}


static int p2p_add_group_clients(struct p2p_data *p2p, const atbm_uint8 *go_dev_addr,
				 const atbm_uint8 *go_interface_addr, int freq,
				 const atbm_uint8 *gi, atbm_size_t gi_len,
				 struct os_reltime *rx_time)
{
	 ...
	 */
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		if (atbm_memcmp(dev->member_in_go_iface, go_interface_addr,
			      ATBM_ETH_ALEN) == 0) {
			atbm_memset(dev->member_in_go_iface, 0, ATBM_ETH_ALEN);
			atbm_memset(dev->member_in_go_dev, 0, ATBM_ETH_ALEN);
		}
	}

	for (c = 0; c < info.num_clients; c++) {
		struct p2p_client_info *cli = &info.client[c];
		if (atbm_memcmp(cli->p2p_device_addr, p2p->cfg->dev_addr,
			      ATBM_ETH_ALEN) == 0)
		...
		atbm_memcpy(dev->interface_addr, cli->p2p_interface_addr,
			  ATBM_ETH_ALEN);
		atbm_memcpy(&dev->last_seen, rx_time, sizeof(struct os_reltime));
		atbm_memcpy(dev->member_in_go_dev, go_dev_addr, ATBM_ETH_ALEN);
		atbm_memcpy(dev->member_in_go_iface, go_interface_addr,
			  ATBM_ETH_ALEN);
...
{
	atbm_memcpy(dev->info.device_name, msg->device_name,
		  sizeof(dev->info.device_name));

	if (msg->manufacturer &&
	    msg->manufacturer_len < sizeof(dev->info.manufacturer)) {
		atbm_memset(dev->info.manufacturer, 0,
			  sizeof(dev->info.manufacturer));
		atbm_memcpy(dev->info.manufacturer, msg->manufacturer,
			  msg->manufacturer_len);
	}

	if (msg->model_name &&
	    msg->model_name_len < sizeof(dev->info.model_name)) {
		atbm_memset(dev->info.model_name, 0,
			  sizeof(dev->info.model_name));
		atbm_memcpy(dev->info.model_name, msg->model_name,
			  msg->model_name_len);
	}

	if (msg->model_number &&
	    msg->model_number_len < sizeof(dev->info.model_number)) {
		atbm_memset(dev->info.model_number, 0,
			  sizeof(dev->info.model_number));
		atbm_memcpy(dev->info.model_number, msg->model_number,
			  msg->model_number_len);
	}

	if (msg->serial_number &&
	    msg->serial_number_len < sizeof(dev->info.serial_number)) {
		atbm_memset(dev->info.serial_number, 0,
			  sizeof(dev->info.serial_number));
		atbm_memcpy(dev->info.serial_number, msg->serial_number,
			  msg->serial_number_len);
	}

	if (msg->pri_dev_type)
		atbm_memcpy(dev->info.pri_dev_type, msg->pri_dev_type,
			  sizeof(dev->info.pri_dev_type));
	else if (msg->wps_pri_dev_type)
		atbm_memcpy(dev->info.pri_dev_type, msg->wps_pri_dev_type,
			  sizeof(dev->info.pri_dev_type));

	if (msg->wps_sec_dev_type_list) {
		atbm_memcpy(dev->info.wps_sec_dev_type_list,
	...
	if (msg->ext_listen_timing) {
		dev->ext_listen_period = ATBM_WPA_GET_LE16(msg->ext_listen_timing);
		dev->ext_listen_interval =
			ATBM_WPA_GET_LE16(msg->ext_listen_timing + 2);
	}

	if (!probe_req) {
		atbm_uint16 new_config_methods;
...
static void p2p_update_peer_vendor_elems(struct p2p_device *dev, const atbm_uint8 *ies,
					 atbm_size_t ies_len)
{
	...
	for (pos = ies; end - pos > 1; pos += len) {
		...
		if (id != ATBM_WLAN_EID_VENDOR_SPECIFIC || len < 3)
			continue;

		if (len >= 4) {
			atbm_uint32 type = ATBM_WPA_GET_BE32(pos);
			      ...
			      const struct p2p_message *msg)
{
	if (dev->info.wfd_subelems && msg->wfd_subelems) {
		...
		return atbm_memcmp(dev->info.wfd_subelems->buf,
 ...
 * Info attributes.
 */
int p2p_add_device(struct p2p_data *p2p, const atbm_uint8 *addr, int freq,
		   struct os_reltime *rx_time, int level, const atbm_uint8 *ies,
		   atbm_size_t ies_len, int scan_res)
{
	struct p2p_device *dev;
	...
	if (!atbm_is_zero_ether_addr(p2p->peer_filter) &&
	    atbm_memcmp(p2p_dev_addr, p2p->peer_filter, ATBM_ETH_ALEN) != 0) {
	...
	//printf("%s:%d,ADDR1:"MACSTR",ADDR2:"MACSTR"\n",__FUNCTION__,__LINE__,
	//	MAC2STR(addr),MAC2STR(p2p_dev_addr));

	//printf("dev->interface_addr:"MACSTR"\n", MAC2STR(dev->interface_addr));
	if (atbm_memcmp(addr, p2p_dev_addr, ATBM_ETH_ALEN) != 0)
		atbm_memcpy(dev->interface_addr, addr, ATBM_ETH_ALEN);
	if (msg.ssid &&
	    msg.ssid[1] <= sizeof(dev->oper_ssid) &&
	    (msg.ssid[1] != P2P_WILDCARD_SSID_LEN ||
	     atbm_memcmp(msg.ssid + 2, P2P_WILDCARD_SSID, P2P_WILDCARD_SSID_LEN)
	     != 0)) {
		atbm_memcpy(dev->oper_ssid, msg.ssid + 2, msg.ssid[1]);
...
static void p2p_search(struct p2p_data *p2p)
{
	...
	atbm_uint16 pw_id = DEV_PW_DEFAULT;
	int res;
...
void p2p_notify_scan_trigger_status(struct p2p_data *p2p, int status)
{
	if (status != 0) {
		...
		atbmwifi_eloop_cancel_timeout(p2p_scan_timeout, p2p, NULL);
		atbmwifi_eloop_register_timeout(P2P_SCAN_TIMEOUT, 0, p2p_scan_timeout,
				       p2p, NULL);
	}
}
...
static void p2p_free_req_dev_types(struct p2p_data *p2p)
{
	...
	atbm_kfree(p2p->req_dev_types);
	p2p->req_dev_types = NULL;
}


static int p2ps_gen_hash(struct p2p_data *p2p, const char *str, atbm_uint8 *hash)
{
	atbm_uint8 buf[SHA256_MAC_LEN];
	char str_buf[256];
	const atbm_uint8 *adv_array;
	atbm_size_t i, adv_len;

	if (!str || !hash)
		return 0;

	if (!str[0]) {
		atbm_memcpy(hash, p2p->wild_card_hash, P2PS_HASH_LEN);
		return 1;
	}

	adv_array = (atbm_uint8 *) str_buf;
	...
	if (atbmwifi_sha256_vector(1, &adv_array, &adv_len, buf))
		return 0;

	atbm_memcpy(hash, buf, P2PS_HASH_LEN);
	return 1;
}
...
int p2p_find(struct p2p_data *p2p, unsigned int timeout,
	     ...
	     atbm_uint8 seek_count, const char **seek, int freq)
{
	int res;
	...
	if (dev_id) {
		atbm_memcpy(p2p->find_dev_id_buf, dev_id, ATBM_ETH_ALEN);
	...
	if (seek_count == 0 || !seek) {
		...
		for (i = 0; i < seek_count; i++) {
			...
			p2p_dbg(p2p, "Seek service %s hash " MACSTR,
				seek[i], MAC2STR(buf));
			atbm_memcpy(&p2p->p2ps_seek_hash[count * P2PS_HASH_LEN],
				  buf, P2PS_HASH_LEN);
			count++;
		}
	...
	if (p2p->p2ps_seek_count == 0 && p2p->p2ps_seek) {
		...
		atbm_memcpy(&p2p->p2ps_seek_hash, p2p->wild_card_hash,
			  P2PS_HASH_LEN);
	}
	...
	atbm_memset(p2p->sd_query_no_ack, 0, ATBM_ETH_ALEN);
	p2p_set_state(p2p, P2P_SEARCH);
	p2p->search_delay = search_delay;
	p2p->in_search_delay = 0;
	atbmwifi_eloop_cancel_timeout(p2p_find_timeout, p2p, NULL);
	p2p->last_p2p_find_timeout = timeout;
	if (timeout)
		atbmwifi_eloop_register_timeout(timeout, 0, p2p_find_timeout,
				       p2p, NULL);
				    ...
				    unsigned int pref_freq, int go)
{
	...
	if (force_freq) {
		...
		atbm_memcpy(&p2p->channels, &p2p->cfg->channels,
...
int p2p_connect(struct p2p_data *p2p, const atbm_uint8 *peer_addr,
		enum p2p_wps_method wps_method,
		int go_intent, const atbm_uint8 *own_interface_addr,
		unsigned int force_freq, int persistent_group,
		const atbm_uint8 *force_ssid, atbm_size_t force_ssid_len,
		int pd_before_go_neg, unsigned int pref_freq, atbm_uint16 oob_pw_id)
{
	struct p2p_device *dev;
	...
	p2p->ssid_set = 0;
	if (force_ssid) {
		p2p_hexdump_ascii(MSG_DEBUG, "P2P: Forced SSID",
				  force_ssid, force_ssid_len);
		atbm_memcpy(p2p->ssid, force_ssid, force_ssid_len);
	...
	atbm_memcpy(p2p->intended_addr, own_interface_addr, ATBM_ETH_ALEN);
	...
	if (p2p->p2p_scan_running) {
		...
		atbm_memcpy(p2p->after_scan_peer, peer_addr, ATBM_ETH_ALEN);
		return 0;
	}
...
int p2p_authorize(struct p2p_data *p2p, const atbm_uint8 *peer_addr,
		  enum p2p_wps_method wps_method,
		  int go_intent, const atbm_uint8 *own_interface_addr,
		  unsigned int force_freq, int persistent_group,
		  const atbm_uint8 *force_ssid, atbm_size_t force_ssid_len,
		  unsigned int pref_freq, atbm_uint16 oob_pw_id)
{
	struct p2p_device *dev;
...
void p2p_add_dev_info(struct p2p_data *p2p, const atbm_uint8 *addr,
...
void p2p_build_ssid(struct p2p_data *p2p, atbm_uint8 *ssid, atbm_size_t *ssid_len)
{
	atbm_memcpy(ssid, P2P_WILDCARD_SSID, P2P_WILDCARD_SSID_LEN);
	p2p_random((char *) &ssid[P2P_WILDCARD_SSID_LEN], 2);
	atbm_memcpy(&ssid[P2P_WILDCARD_SSID_LEN + 2],
...
{
	if (p2p->ssid_set) {
		atbm_memcpy(params->ssid, p2p->ssid, p2p->ssid_len);
...
void p2p_go_complete(struct p2p_data *p2p, struct p2p_device *peer)
{
	...
	if (go) {
		...
		res.freq = p2p_channel_to_freq(p2p->op_reg_class,
					       p2p->op_channel);
		atbm_memcpy(res.ssid, p2p->ssid, p2p->ssid_len);
...
static int p2p_rx_p2p_action(struct p2p_data *p2p, const atbm_uint8 *sa,
			      atbm_uint8 *data, atbm_size_t len, int rx_freq)
{
	atbm_uint8 p2p_type = 0;
	p2p_dbg(p2p, "RX P2P Public Action from " MACSTR, MAC2STR(sa));
	p2p_hexdump(MSG_MSGDUMP, "P2P: P2P Public Action contents", data, len);
	...
	switch (p2p_type) {
	case P2P_GO_NEG_REQ:
		p2p_process_go_neg_req(p2p, sa, data, len, rx_freq);
		break;
	case P2P_GO_NEG_RESP:
		p2p_process_go_neg_resp(p2p, sa, data, len, rx_freq);
		break;
	case P2P_GO_NEG_CONF:
		p2p_process_go_neg_conf(p2p, sa, data, len);
		break;
	case P2P_INVITATION_REQ:
		p2p_process_invitation_req(p2p, sa, data, len,
					   rx_freq);
		break;
	case P2P_INVITATION_RESP:
		p2p_process_invitation_resp(p2p, sa, data, len );
		break;
	case P2P_DEV_DISC_REQ:
		p2p_process_dev_disc_req(p2p, sa, data, len, rx_freq);
		break;
	case P2P_DEV_DISC_RESP:
		p2p_process_dev_disc_resp(p2p, sa, data, len);
		break;
	case P2P_PROV_DISC_REQ:
		p2p_process_prov_disc_req(p2p, sa, data, len, rx_freq);
		break;
	case P2P_PROV_DISC_RESP:
		p2p_process_prov_disc_resp(p2p, sa, data, len);
		break;
	default:
...
static void p2p_rx_action_public(struct p2p_data *p2p, const atbm_uint8 *da,
				 const atbm_uint8 *sa, const atbm_uint8 *bssid, atbm_uint8 *data,
				 atbm_size_t len, int freq)
{
	if (len < 1)
		return;
	...
	case WLAN_PA_VENDOR_SPECIFIC:
		...
		if (len < 4)
			return;
		if (ATBM_WPA_GET_BE32(data) != P2P_IE_VENDOR_TYPE)
			return;
...
void p2p_rx_action(struct p2p_data *p2p, const atbm_uint8 *da, const atbm_uint8 *sa,
		   const atbm_uint8 *bssid, atbm_uint8 category,
		   atbm_uint8 *data, atbm_size_t len, int freq)
{
	if (category == WLAN_ACTION_PUBLIC) {
		//printf("rx WLAN_ACTION_PUBLIC\n");
	...
	p2p_hexdump(MSG_MSGDUMP, "P2P: P2P Action contents", data, len);
...
static void p2p_add_dev_from_probe_req(struct p2p_data *p2p, const atbm_uint8 *addr,
				       const atbm_uint8 *ie, atbm_size_t ie_len)
{
	...
	if (dev) {
		if (msg.listen_channel) {
			...
			if (dev->country[0] == 0)
				atbm_memcpy(dev->country, msg.listen_channel, 3);
...
static int dev_type_match(const atbm_uint8 *dev_type, const atbm_uint8 *req_dev_type)
{
	if (atbm_memcmp(dev_type, req_dev_type, WPS_DEV_TYPE_LEN) == 0)
		return 1;
	if (atbm_memcmp(dev_type, req_dev_type, 2) == 0 &&
	    ATBM_WPA_GET_BE32(&req_dev_type[2]) == 0 &&
	    ATBM_WPA_GET_BE16(&req_dev_type[6]) == 0)
		return 1; /* Category match with wildcard OUI/sub-category */
	return 0;
}


int dev_type_list_match(const atbm_uint8 *dev_type, const atbm_uint8 *req_dev_type[],
...
struct wpabuf * p2p_build_probe_resp_ies(struct p2p_data *p2p,
					 const atbm_uint8 *query_hash,
					 atbm_uint8 query_count)
{
	struct wpabuf *buf;
	atbm_uint8 *len;
	int pw_id = -1;
...
static int p2p_build_probe_resp_buf(struct p2p_data *p2p, struct wpabuf *buf,
				    struct wpabuf *ies,
				    const atbm_uint8 *addr, int rx_freq)
{
	struct atbmwifi_ieee80211_mgmt *resp;
	atbm_uint8 channel, op_class;

	resp = wpabuf_put(buf, offsetof(struct atbmwifi_ieee80211_mgmt,
					u.probe_resp.variable));

	resp->frame_control = atbm_host_to_le16(ATBM_IEEE80211_FTYPE_MGMT |
					   ATBM_IEEE80211_STYPE_PROBE_RESP);
	atbm_memcpy(resp->da, addr, ATBM_ETH_ALEN);
	atbm_memcpy(resp->sa, p2p->cfg->dev_addr, ATBM_ETH_ALEN);
	atbm_memcpy(resp->bssid, p2p->cfg->dev_addr, ATBM_ETH_ALEN);
	resp->u.probe_resp.beacon_int = atbm_host_to_le16(100);
	/* hardware or low-level driver will setup seq_ctrl and timestamp */
	resp->u.probe_resp.capab_info =
	    atbm_host_to_le16(ATBM_WLAN_CAPABILITY_SHORT_PREAMBLE |
		     ATBM_WLAN_CAPABILITY_PRIVACY |
		     ATBM_WLAN_CAPABILITY_SHORT_SLOT_TIME);

	wpabuf_put_u8(buf, ATBM_WLAN_EID_SSID);
	wpabuf_put_u8(buf, P2P_WILDCARD_SSID_LEN);
	wpabuf_put_data(buf, P2P_WILDCARD_SSID, P2P_WILDCARD_SSID_LEN);

	wpabuf_put_u8(buf, ATBM_WLAN_EID_SUPP_RATES);
	wpabuf_put_u8(buf, 8);
	...
	wpabuf_put_u8(buf, ATBM_WLAN_EID_DS_PARAMS);
	wpabuf_put_u8(buf, 1);
...
static int p2p_service_find_asp(struct p2p_data *p2p, const atbm_uint8 *hash)
{
	...
	any_wfa = atbm_memcmp(hash, p2p->wild_card_hash, P2PS_HASH_LEN) == 0;

	adv_data = p2p->p2ps_adv_list;
	while (adv_data) {
		if (atbm_memcmp(hash, adv_data->hash, P2PS_HASH_LEN) == 0)
...
p2p_reply_probe(struct p2p_data *p2p, const atbm_uint8 *addr, const atbm_uint8 *dst,
		const atbm_uint8 *bssid, const atbm_uint8 *ie, atbm_size_t ie_len,
		unsigned int rx_freq)
{
	...
	struct wpabuf *ies;
#if 0
	if (atbm_ieee802_11_parse_elems((atbm_uint8 *) ie, ie_len, &elems) ==
	    ParseFailed) {
...
#endif
	atbm_ieee802_11_parse_elems((atbm_uint8 *) ie, ie_len, &elems);

	if (elems.p2p_ie == NULL) {
		/* not a P2P probe - ignore it */
		p2p_dbg(p2p, "Not a P2P probe - ignore it");
		return P2P_PREQ_NOT_P2P;
	}

	if (dst && !atbm_is_broadcast_ether_addr(dst) &&
	    atbm_memcmp(dst, p2p->cfg->dev_addr, ATBM_ETH_ALEN) != 0) {
		/* Not sent to the broadcast address or our P2P Device Address
		 */
		p2p_info(p2p, "Probe Req DA " MACSTR " not ours - ignore it",
			MAC2STR(dst));
		return P2P_PREQ_NOT_PROCESSED;
	}

	if (bssid && !atbm_is_broadcast_ether_addr(bssid)) {
		/* Not sent to the Wildcard BSSID */
		p2p_info(p2p, "Probe Req BSSID " MACSTR " not wildcard - ignore it",
			MAC2STR(bssid));
		return P2P_PREQ_NOT_PROCESSED;
	}

	if (elems.ssid == NULL || elems.ssid_len != P2P_WILDCARD_SSID_LEN ||
	    atbm_memcmp(elems.ssid, P2P_WILDCARD_SSID, P2P_WILDCARD_SSID_LEN) !=
	    0) {
		/* not using P2P Wildcard SSID - ignore */
		p2p_info(p2p, "Probe Req not using P2P Wildcard SSID - ignore it");
		return P2P_PREQ_NOT_PROCESSED;
	}

	if (supp_rates_11b_only(&elems)) {
		/* Indicates support for 11b rates only */
		p2p_info(p2p, "Probe Req with 11b rates only supported - ignore it");
		return P2P_PREQ_NOT_P2P;
	}
	...
	if (p2p_parse_ies(ie, ie_len, &msg) < 0) {
		...
		p2p_info(p2p, "Could not parse P2P attributes in Probe Req - ignore it");
		return P2P_PREQ_NOT_P2P;
	}

	if (msg.service_hash && msg.service_hash_count) {
		const atbm_uint8 *hash = msg.service_hash;
		atbm_uint8 i;
		...
		if (!p2p->in_listen && !p2p->drv_in_listen &&
			...
			p2p_info(p2p, "Do not reply to Probe Request frame that was received on %u MHz while waiting to start Listen state on %u MHz",
				rx_freq, p2p->pending_listen_freq);
			p2p_parse_free(&msg);
		...
		/* Probed hash unknown */
		if (!p2ps_svc_found) {
			p2p_info(p2p, "No Service Hash match found");
			p2p_parse_free(&msg);
			return P2P_PREQ_NOT_PROCESSED;
		}
	} else {
		...
		if (!p2p->in_listen || !p2p->drv_in_listen) {
			...
			p2p_info(p2p, "Not in Listen state (in_listen=%d drv_in_listen=%d) - ignore Probe Request",
	...
	if (msg.device_id &&
	    atbm_memcmp(msg.device_id, p2p->cfg->dev_addr, ATBM_ETH_ALEN) != 0) {
		/* Device ID did not match */
		p2p_info(p2p, "Probe Req requested Device ID " MACSTR " did not match - ignore it",
	...
	if (msg.wps_attributes &&
		...
		p2p_info(p2p, "Probe Req requested Device Type did not match - ignore it");
		p2p_parse_free(&msg);
		return P2P_PREQ_NOT_PROCESSED;
	}
	...
	if (!p2p->cfg->send_probe_resp) {
		...
		p2p_info(p2p, "Probe Resp generated elsewhere - do not generate additional response");
		p2p_parse_free(&msg);
		return P2P_PREQ_NOT_PROCESSED;
	}
...
p2p_probe_req_rx(struct p2p_data *p2p, const atbm_uint8 *addr, const atbm_uint8 *dst,
		 const atbm_uint8 *bssid, const atbm_uint8 *ie, atbm_size_t ie_len,
		 unsigned int rx_freq, int p2p_lo_started)
{
	...
	if ((p2p->state == P2P_CONNECT || p2p->state == P2P_CONNECT_LISTEN) &&
	    p2p->go_neg_peer &&
	    atbm_memcmp(addr, p2p->go_neg_peer->info.p2p_device_addr, ATBM_ETH_ALEN)
	    == 0 &&
	    !(p2p->go_neg_peer->flags & P2P_DEV_WAIT_GO_NEG_CONFIRM)) {
		/* Received a Probe Request from GO Negotiation peer */
		p2p_dbg(p2p, "Found GO Negotiation peer - try to start GO negotiation from timeout");
//		printf("%s:%d\n",__FUNCTION__,__LINE__);
//		atbmwifi_eloop_cancel_timeout(p2p_go_neg_start, p2p, NULL);
//		atbmwifi_eloop_register_timeout(0, 0, p2p_go_neg_start, p2p, NULL);
		return res;
	}
	...
	if ((p2p->state == P2P_INVITE || p2p->state == P2P_INVITE_LISTEN) &&
	    ...
	    atbm_memcmp(addr, p2p->invite_peer->info.p2p_device_addr, ATBM_ETH_ALEN)
	    == 0) {
		/* Received a Probe Request from Invite peer */
		p2p_dbg(p2p, "Found Invite peer - try to start Invite from timeout");
		atbmwifi_eloop_cancel_timeout(p2p_invite_start, p2p, NULL);
		atbmwifi_eloop_register_timeout(0, 0, p2p_invite_start, p2p, NULL);
		return res;
	}
...
static int p2p_assoc_req_ie_wlan_ap(struct p2p_data *p2p, const atbm_uint8 *bssid,
				    atbm_uint8 *buf, atbm_size_t len, struct wpabuf *p2p_ie)
{
	struct wpabuf *tmp;
	atbm_uint8 *lpos;
	atbm_size_t tmplen;
	int res;
	atbm_uint8 group_capab;
	struct p2p_message msg;
	...
	if (tmplen > len)
		...
		atbm_memcpy(buf, wpabuf_head(tmp), tmplen);
		res = tmplen;
	}
	wpabuf_free(tmp);
...
int p2p_assoc_req_ie(struct p2p_data *p2p, const atbm_uint8 *bssid, atbm_uint8 *buf,
		     atbm_size_t len, int p2p_group, struct wpabuf *p2p_ie)
{
	struct wpabuf *tmp;
	atbm_uint8 *lpos;
...
int p2p_scan_result_text(const atbm_uint8 *ies, atbm_size_t ies_len, char *buf, char *end)
{
...
p2p_service_p2ps_id(struct p2p_data *p2p, atbm_uint32 adv_id)
{
	struct p2ps_advertisement *adv_data;
...
int p2p_service_del_asp(struct p2p_data *p2p, atbm_uint32 adv_id)
{
	struct p2ps_advertisement *adv_data;
...
int p2p_service_add_asp(struct p2p_data *p2p, int auto_accept, atbm_uint32 adv_id,
			const char *adv_str, atbm_uint8 svc_state, atbm_uint16 config_methods,
			const char *svc_info, const atbm_uint8 *cpt_priority)
{
	struct p2ps_advertisement *adv_data, *tmp, **prev;
	atbm_uint8 buf[P2PS_HASH_LEN];
	atbm_size_t adv_data_len, adv_len, info_len = 0;
	int i;
	...
	adv_data = (struct p2ps_advertisement *)atbm_kzalloc(adv_data_len, GFP_KERNEL);
	if (!adv_data)
		return -1;

	atbm_memcpy(adv_data->hash, buf, P2PS_HASH_LEN);
	adv_data->id = adv_id;
	adv_data->state = svc_state;
	adv_data->config_methods = config_methods & p2p->cfg->config_methods;
	adv_data->auto_accept = (atbm_uint8) auto_accept;
	atbm_memcpy(adv_data->svc_name, adv_str, adv_len);
	...
	if (svc_info && info_len) {
		...
		atbm_memcpy(adv_data->svc_info, svc_info, info_len);
	}
	...
	while (tmp) {
		if (tmp->id == adv_data->id) {
			if (strcmp(tmp->svc_name, adv_data->svc_name) != 0) {
				atbm_kfree(adv_data);
				return -1;
			}
...
int p2p_parse_dev_addr_in_p2p_ie(struct wpabuf *p2p_ie, atbm_uint8 *dev_addr)
{
	struct p2p_message msg;
	...
	if (msg.p2p_device_addr) {
		atbm_memcpy(dev_addr, msg.p2p_device_addr, ATBM_ETH_ALEN);
		return 0;
	} else if (msg.device_id) {
		atbm_memcpy(dev_addr, msg.device_id, ATBM_ETH_ALEN);
		return 0;
	}
	return -1;
}
...
int p2p_parse_dev_addr(const atbm_uint8 *ies, atbm_size_t ies_len, atbm_uint8 *dev_addr)
{
	struct wpabuf *p2p_ie;
	int ret;
...
void p2p_wps_success_cb(struct p2p_data *p2p, const atbm_uint8 *mac_addr)
{
	if (p2p->go_neg_peer == NULL) {
	...
	if (atbm_memcmp(mac_addr, p2p->go_neg_peer->intended_addr, ATBM_ETH_ALEN) !=
	    0) {
...
struct p2p_data * p2p_init(const struct p2p_config *cfg)
{
	...
	p2p = (struct p2p_data *)atbm_kzalloc(sizeof(*p2p) + sizeof(*cfg), GFP_KERNEL);
	if (p2p == NULL)
		return NULL;
	p2p->cfg = (struct p2p_config *) (p2p + 1);
	atbm_memcpy(p2p->cfg, cfg, sizeof(*cfg));
	if (cfg->dev_name)
	...
	if (cfg->pref_chan) {
		p2p->cfg->pref_chan = (struct p2p_channel *)atbm_kmalloc(cfg->num_pref_chan *
						sizeof(struct p2p_channel), GFP_KERNEL);
		if (p2p->cfg->pref_chan) {
			atbm_memcpy(p2p->cfg->pref_chan, cfg->pref_chan,
				  cfg->num_pref_chan *
	...
	if (atbmwifi_os_get_random(&p2p->next_tie_breaker, 1) < 0)
...
void p2p_deinit(struct p2p_data *p2p)
{
	...
	p2p_flush(p2p);
	p2p_free_req_dev_types(p2p);
	atbm_kfree(p2p->cfg->dev_name);
	atbm_kfree(p2p->cfg->manufacturer);
	atbm_kfree(p2p->cfg->model_name);
	atbm_kfree(p2p->cfg->model_number);
	atbm_kfree(p2p->cfg->serial_number);
	atbm_kfree(p2p->cfg->pref_chan);
	atbm_kfree(p2p->groups);
	p2ps_prov_free(p2p);
	wpabuf_free(p2p->sd_resp);
	p2p_remove_wps_vendor_extensions(p2p);
	atbm_kfree(p2p->no_go_freq.range);
...
int p2p_unauthorize(struct p2p_data *p2p, const atbm_uint8 *addr)
{
	struct p2p_device *dev;
...
void p2p_set_config_methods(struct p2p_data *p2p, atbm_uint16 config_methods)
{
	p2p->cfg->config_methods = config_methods;
}


void p2p_set_uuid(struct p2p_data *p2p, const atbm_uint8 *uuid)
{
	atbm_memcpy(p2p->cfg->uuid, uuid, 16);
}


int p2p_set_pri_dev_type(struct p2p_data *p2p, const atbm_uint8 *pri_dev_type)
{
	atbm_memcpy(p2p->cfg->pri_dev_type, pri_dev_type, 8);
	return 0;
}


int p2p_set_sec_dev_types(struct p2p_data *p2p, const atbm_uint8 dev_types[][8],
			  atbm_size_t num_dev_types)
{
	if (num_dev_types > P2P_SEC_DEVICE_TYPES)
		num_dev_types = P2P_SEC_DEVICE_TYPES;
	p2p->cfg->num_sec_dev_types = num_dev_types;
	atbm_memcpy(p2p->cfg->sec_dev_type, dev_types, num_dev_types * 8);
	return 0;
}
...
{
	atbm_memcpy(p2p->cfg->country, country, 3);
	return 0;
}
...
static void p2p_sd_cb(struct p2p_data *p2p, int success)
{
	...
	if (!success) {
		if (p2p->sd_peer) {
			if (atbm_is_zero_ether_addr(p2p->sd_query_no_ack)) {
				atbm_memcpy(p2p->sd_query_no_ack,
...
static void p2p_retry_pd(struct p2p_data *p2p)
{
	...
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		if (atbm_memcmp(p2p->pending_pd_devaddr,
			      dev->info.p2p_device_addr, ATBM_ETH_ALEN) != 0)
			continue;
		if (!dev->req_config_methods)
			continue;
...
int p2p_scan_res_handler(struct p2p_data *p2p, const atbm_uint8 *bssid, int freq,
			 struct os_reltime *rx_time, int level, const atbm_uint8 *ies,
			 atbm_size_t ies_len)
{
...
void p2p_scan_ie(struct p2p_data *p2p, struct wpabuf *ies, const atbm_uint8 *dev_id,
		 unsigned int bands)
{
	atbm_uint8 dev_capab;
	atbm_uint8 *len;
...
atbm_size_t p2p_scan_ie_buf_len(struct p2p_data *p2p)
{
	atbm_size_t len = 100;
...
static void p2p_go_neg_req_cb(struct p2p_data *p2p, int success)
{
	...
	if (!success &&
	    (dev->info.dev_capab & P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY) &&
	    !atbm_is_zero_ether_addr(dev->member_in_go_dev)) {
				       ...
				       const atbm_uint8 *addr)
{
	...
	if (success) {
		...
		if (dev && dev->status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE){
			...
			if(p2p->cfg->peer_waiting_cb){
				if(0 == p2p->cfg->peer_waiting_cb(p2p->cfg->cb_ctx, addr)){
					return;
				}
			}
		}	
...
void p2p_send_action_cb(struct p2p_data *p2p, unsigned int freq, const atbm_uint8 *dst,
			const atbm_uint8 *src, const atbm_uint8 *bssid,
			enum p2p_send_action_result result)
{
...
//	printf("action_cb state:%d,success:%d\n", state, success);
	switch (state) {
		   ...
		   unsigned int duration)
{
	...
	if (p2p->pending_listen_sec || p2p->pending_listen_usec) {
		...
		p2p_set_timeout(p2p, p2p->pending_listen_sec,
				p2p->pending_listen_usec + 200000);
	}
...
int p2p_listen_end(struct p2p_data *p2p, unsigned int freq)
{
	...
	if (p2p->state == P2P_CONNECT_LISTEN && p2p->go_neg_peer) {
		...
		if (p2p->pending_listen_freq) {
			/*
			 * Better wait a bit ifs the driver is unable to start
...
{
	atbm_uint32 adv_id = 0;
	atbm_uint8 *adv_mac = NULL;
...
static void p2p_state_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (p2p->drv_in_listen) {
		...
		p2p->cfg->stop_listen(p2p->cfg->cb_ctx);
		if(p2p->state == P2P_WAIT_PEER_CONNECT){
			p2p_set_state(p2p, P2P_CONNECT);
		}else{
			return;
		}
...
int p2p_reject(struct p2p_data *p2p, const atbm_uint8 *peer_addr)
{
	struct p2p_device *dev;
...
const struct p2p_peer_info * p2p_get_peer_info(struct p2p_data *p2p,
					       const atbm_uint8 *addr, int next)
{
	struct p2p_device *dev;
			  ...
			  char *buf, atbm_size_t buflen)
{
	...
	dev = (struct p2p_device *) (((atbm_uint8 *) info) -
	...
	if (dev->info.wfd_subelems) {
		...
		pos += atbmwifi_wpa_snprintf_hex(pos, end - pos,
...
int p2p_peer_known(struct p2p_data *p2p, const atbm_uint8 *addr)
{
...
static struct wpabuf * p2p_build_presence_req(atbm_uint32 duration1, atbm_uint32 interval1,
					      atbm_uint32 duration2, atbm_uint32 interval2)
{
	struct wpabuf *req;
	...
	if (duration1 || interval1) {
		atbm_memset(&desc1, 0, sizeof(desc1));
		desc1.count_type = 1;
		...
		if (duration2 || interval2) {
			atbm_memset(&desc2, 0, sizeof(desc2));
			desc2.count_type = 2;
...
int p2p_presence_req(struct p2p_data *p2p, const atbm_uint8 *go_interface_addr,
		     const atbm_uint8 *own_interface_addr, unsigned int freq,
		     atbm_uint32 duration1, atbm_uint32 interval1, atbm_uint32 duration2,
		     atbm_uint32 interval2)
{
	struct wpabuf *req;
...
static struct wpabuf * p2p_build_presence_resp(atbm_uint8 status, const atbm_uint8 *noa,
					       atbm_size_t noa_len, atbm_uint8 dialog_token)
{
	struct wpabuf *resp;
	atbm_uint8 *len;
				     ...
				     int rx_freq)
{
	...
	for (g = 0; g < p2p->num_groups; g++) {
		if (atbm_memcmp(da, p2p_group_get_interface_addr(p2p->groups[g]),
			      ATBM_ETH_ALEN) == 0) {
			group = p2p->groups[g];
			break;
		}
				      ...
				      const atbm_uint8 *sa, const atbm_uint8 *data, atbm_size_t len)
{
	...
	p2p_hexdump(MSG_DEBUG, "P2P: P2P Presence Response - NoA",
		    msg.noa, msg.noa_len);
	/* TODO: process NoA */
	p2p_parse_free(&msg);
}
...
static void p2p_ext_listen_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (p2p->ext_listen_interval) {
		...
		atbmwifi_eloop_register_timeout(p2p->ext_listen_interval_sec, 
						p2p->ext_listen_interval_usec/1000,
		   ...
		   unsigned int interval)
{
	...
	atbmwifi_eloop_cancel_timeout(p2p_ext_listen_timeout, p2p, NULL);
...
void p2p_deauth_notif(struct p2p_data *p2p, const atbm_uint8 *bssid, atbm_uint16 reason_code,
		      const atbm_uint8 *ie, atbm_size_t ie_len)
{
	struct p2p_message msg;
...
void p2p_disassoc_notif(struct p2p_data *p2p, const atbm_uint8 *bssid, atbm_uint16 reason_code,
			const atbm_uint8 *ie, atbm_size_t ie_len)
{
	struct p2p_message msg;
...
int p2p_config_get_random_social(struct p2p_config *p2p, atbm_uint8 *op_class,
				 atbm_uint8 *op_channel,
...
int p2p_set_listen_channel(struct p2p_data *p2p, atbm_uint8 reg_class, atbm_uint8 channel,
			   atbm_uint8 forced)
{
...
atbm_uint8 p2p_get_listen_channel(struct p2p_data *p2p)
{
	return p2p->cfg->channel;
}


int p2p_set_ssid_postfix(struct p2p_data *p2p, const atbm_uint8 *postfix, atbm_size_t len)
{
	...
	if (len > sizeof(p2p->cfg->ssid_postfix))
		return -1;
	atbm_memcpy(p2p->cfg->ssid_postfix, postfix, len);
	p2p->cfg->ssid_postfix_len = len;
	return 0;
}


int p2p_set_oper_channel(struct p2p_data *p2p, atbm_uint8 op_reg_class, atbm_uint8 op_channel,
			 int cfg_op_channel)
{
		       ...
		       const struct wpa_freq_range_list *list)
{
	...
	if (tmp == NULL)
		return -1;
	atbm_memcpy(tmp, list->range, list->num * sizeof(struct wpa_freq_range));
	atbm_kfree(p2p->no_go_freq.range);
...
int p2p_get_interface_addr(struct p2p_data *p2p, const atbm_uint8 *dev_addr,
			   atbm_uint8 *iface_addr)
{
	struct p2p_device *dev = p2p_get_device(p2p, dev_addr);
	if (dev == NULL || atbm_is_zero_ether_addr(dev->interface_addr))
		return -1;
	atbm_memcpy(iface_addr, dev->interface_addr, ATBM_ETH_ALEN);
	return 0;
}


int p2p_get_dev_addr(struct p2p_data *p2p, const atbm_uint8 *iface_addr,
			   atbm_uint8 *dev_addr)
{
	struct p2p_device *dev = p2p_get_device_interface(p2p, iface_addr);
	if (dev == NULL)
		return -1;
	atbm_memcpy(dev_addr, dev->info.p2p_device_addr, ATBM_ETH_ALEN);
	return 0;
}


void p2p_set_peer_filter(struct p2p_data *p2p, const atbm_uint8 *addr)
{
	atbm_memcpy(p2p->peer_filter, addr, ATBM_ETH_ALEN);
	if (atbm_is_zero_ether_addr(p2p->peer_filter))
...
int p2p_get_oper_freq(struct p2p_data *p2p, const atbm_uint8 *iface_addr)
{
	struct p2p_device *dev = p2p_get_device_interface(p2p, iface_addr);
	if (dev == NULL){
		printf("no found dev\n");
			     ...
			     const struct p2p_channels *cli_chan)
{
	...
	atbm_memcpy(&p2p->cfg->channels, chan, sizeof(struct p2p_channels));
	p2p_channels_dump(p2p, "channels", &p2p->cfg->channels);
	atbm_memcpy(&p2p->cfg->cli_channels, cli_chan,
		  sizeof(struct p2p_channels));
	p2p_channels_dump(p2p, "cli_channels", &p2p->cfg->cli_channels);
}


int p2p_send_action(struct p2p_data *p2p, unsigned int freq, const atbm_uint8 *dst,
		    const atbm_uint8 *src, const atbm_uint8 *bssid, const atbm_uint8 *buf,
		    atbm_size_t len, unsigned int wait_time)
{
	int res, scheduled;
	int i;
...
const atbm_uint8 * p2p_get_go_neg_peer(struct p2p_data *p2p)
{
...
p2p_get_peer_found(struct p2p_data *p2p, const atbm_uint8 *addr, int next)
{
	struct p2p_device *dev;
...
void p2p_set_config_timeout(struct p2p_data *p2p, atbm_uint8 go_timeout,
			    atbm_uint8 client_timeout)
{
	if (p2p) {
...
static struct wpabuf * p2p_build_nfc_handover(struct p2p_data *p2p,
					      int client_freq,
					      const atbm_uint8 *go_dev_addr,
					      const atbm_uint8 *ssid, atbm_size_t ssid_len)
{
	struct wpabuf *buf;
	atbm_uint8 op_class, channel;
	...
	if (p2p->num_groups > 0) {
		...
		p2p_group_buf_add_id(p2p->groups[0], buf);
		   ...
		   go_dev_addr && !atbm_is_zero_ether_addr(go_dev_addr) &&
		   ssid && ssid_len > 0) {
		/*
					...
					struct p2p_nfc_params *params)
{
	...
	if (msg.oob_dev_password) {
		atbm_memcpy(params->oob_dev_pw, msg.oob_dev_password,
	...
	if (msg.group_id) {
		atbm_memcpy(params->go_dev_addr, msg.group_id, ATBM_ETH_ALEN);
		params->go_ssid_len = msg.group_id_len - ATBM_ETH_ALEN;
		atbm_memcpy(params->go_ssid, msg.group_id + ATBM_ETH_ALEN,
			  params->go_ssid_len);
	}
...
void p2p_set_authorized_oob_dev_pw_id(struct p2p_data *p2p, atbm_uint16 dev_pw_id,
				      int go_intent,
				      const atbm_uint8 *own_interface_addr)
{
...
void p2p_set_override_pref_op_chan(struct p2p_data *p2p, atbm_uint8 op_class,
				   atbm_uint8 chan)
{
					      ...
					      unsigned int freq)
{
	...
	atbm_uint8 addr[] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
	int ret;
