...
struct p2p_group_member {
	...
	atbm_uint8 addr[ATBM_ETH_ALEN]; /* P2P Interface Address */
	atbm_uint8 dev_addr[ATBM_ETH_ALEN]; /* P2P Device Address */
				  ...
				  struct p2p_group_config *config)
{
	...
	group = (struct p2p_group *)atbm_kzalloc(sizeof(*group), GFP_KERNEL);
	if (group == NULL)
		return NULL;

	if(!p2p->groups){
		groups = p2p->groups = (struct p2p_group **)atbm_kzalloc(P2P_MAX_GROUP_ENTRIES * sizeof(struct p2p_group *), GFP_KERNEL);
	}
...
static void p2p_group_add_common_ies(struct p2p_group *group,
				     struct wpabuf *ie)
{
	atbm_uint8 dev_capab = group->p2p->dev_capab, group_capab = 0;
...
{
	const atbm_uint8 *pos, *end;
	const atbm_uint8 *dev_info = NULL;
	const atbm_uint8 *assoc_bssid = NULL;
	const atbm_uint8 *coupled_sink = NULL;
	atbm_uint8 zero_addr[ATBM_ETH_ALEN];

	if (m->wfd_ie == NULL)
		return 0;

	atbm_memset(zero_addr, 0, ATBM_ETH_ALEN);
	...
	wpabuf_put_data(buf, m->dev_addr, ATBM_ETH_ALEN);
	if (assoc_bssid)
		wpabuf_put_data(buf, assoc_bssid, ATBM_ETH_ALEN);
	else
		wpabuf_put_data(buf, zero_addr, ATBM_ETH_ALEN);
	wpabuf_put_data(buf, dev_info, 2); /* WFD Device Info */
	wpabuf_put_data(buf, dev_info + 4, 2); /* WFD Device Max Throughput */
	if (coupled_sink) {
		wpabuf_put_data(buf, coupled_sink, 1 + ATBM_ETH_ALEN);
	} else {
		wpabuf_put_u8(buf, 0);
		wpabuf_put_data(buf, zero_addr, ATBM_ETH_ALEN);
	}
...
wifi_display_build_go_ie(struct p2p_group *group)
{
	...
	if (count == 0) {
		...
		ATBM_WPA_PUT_BE16(len, (atbm_uint8 *) wpabuf_put(wfd_subelems, 0) - len -
			     2);
			    ...
			    int max_clients)
{
	...
	ATBM_WPA_PUT_LE16(group_info + 1,
		     (atbm_uint8 *) wpabuf_put(buf, 0) - group_info - 3);
}
...
void p2p_group_update_ies(struct p2p_group *group)
{
	...
	if (probe_resp_ie == NULL)
		return;
	p2p_hexdump_buf(MSG_MSGDUMP, "P2P: Update GO Probe Response P2P IE",
			probe_resp_ie);
	...
	if (group->beacon_update) {
		...
		p2p_hexdump_buf(MSG_MSGDUMP, "P2P: Update GO Beacon P2P IE",
				beacon_ie);
	} else
		beacon_ie = NULL;
 ...
 */
static struct wpabuf * p2p_build_client_info(const atbm_uint8 *addr,
					     struct wpabuf *p2p_ie,
					     atbm_uint8 *dev_capab, atbm_uint8 *dev_addr)
{
	const atbm_uint8 *spos;
	...
	buf = wpabuf_alloc(ATBM_ETH_ALEN + 1 + 1 + msg.p2p_device_info_len);
	if (buf == NULL)
		return NULL;
	...
	*dev_capab = msg.capability[0];
	...
	/* P2P Device address */
	wpabuf_put_data(buf, spos, ATBM_ETH_ALEN);
	/* P2P Interface address */
	wpabuf_put_data(buf, addr, ATBM_ETH_ALEN);
	 ...
	 */
	wpabuf_put_data(buf, spos + ATBM_ETH_ALEN,
			msg.p2p_device_info_len - ATBM_ETH_ALEN);
...
static int p2p_group_remove_member(struct p2p_group *group, const atbm_uint8 *addr)
{
	struct p2p_group_member *m, *prev;
	...
	prev = NULL;
	while (m) {
		if (atbm_memcmp(m->addr, addr, ATBM_ETH_ALEN) == 0)
			break;
		prev = m;
		m = m->next;
	}
			  ...
			  const atbm_uint8 *ie, atbm_size_t len)
{
	...
	m = (struct p2p_group_member *)atbm_kzalloc(sizeof(*m), GFP_KERNEL);
	if (m == NULL)
		return -1;
	atbm_memcpy(m->addr, addr, ATBM_ETH_ALEN);
...
int p2p_group_match_dev_id(struct p2p_group *group, struct wpabuf *p2p)
{
	...
	if (atbm_memcmp(msg.device_id, group->p2p->cfg->dev_addr, ATBM_ETH_ALEN) == 0)
		return 1; /* Match with our P2P Device Address */

	for (m = group->members; m; m = m->next) {
		if (atbm_memcmp(msg.device_id, m->dev_addr, ATBM_ETH_ALEN) == 0)
...
static struct p2p_group_member * p2p_group_get_client(struct p2p_group *group,
						      const atbm_uint8 *dev_id)
{
	struct p2p_group_member *m;

	for (m = group->members; m; m = m->next) {
		if (atbm_memcmp(dev_id, m->dev_addr, ATBM_ETH_ALEN) == 0)
			return m;
	}
...
static struct p2p_group_member * p2p_group_get_client_iface(
	struct p2p_group *group, const atbm_uint8 *interface_addr)
{
	struct p2p_group_member *m;

	for (m = group->members; m; m = m->next) {
		if (atbm_memcmp(interface_addr, m->addr, ATBM_ETH_ALEN) == 0)
			return m;
	}
...
const atbm_uint8 * p2p_group_get_dev_addr(struct p2p_group *group, const atbm_uint8 *addr)
{
	...
	if (m && !atbm_is_zero_ether_addr(m->dev_addr))
		return m->dev_addr;
	return NULL;
}
			  ...
			  const atbm_uint8 *noa, atbm_size_t noa_len)
{
	...
	p2p_hexdump(MSG_DEBUG, "P2P: Presence Request NoA", noa, noa_len);
	...
	if (curr_noa_len < 0)
		...
		p2p_hexdump(MSG_DEBUG, "P2P: Current NoA", curr_noa,
			    curr_noa_len);
...
int p2p_group_is_client_connected(struct p2p_group *group, const atbm_uint8 *dev_addr)
{
	...
	for (m = group->members; m; m = m->next) {
		if (atbm_memcmp(m->dev_addr, dev_addr, ATBM_ETH_ALEN) == 0)
			return 1;
	}
...
{
	if (group_id_len != ATBM_ETH_ALEN + group->cfg->ssid_len)
		return 0;
	if (atbm_memcmp(group_id, group->p2p->cfg->dev_addr, ATBM_ETH_ALEN) != 0)
		return 0;
	return atbm_memcmp(group_id + ATBM_ETH_ALEN, group->cfg->ssid,
...

{
	...
	atbm_memset(&intersect, 0, sizeof(intersect));
	...
	atbm_memset(common_freqs, 0, *num * sizeof(int));
