 ...
 */
#define P2P_MAX_PREF_CHANNELS 32

/**
 * P2P_MAX_REG_CLASSES - Maximum number of regulatory classes
 */
#define P2P_MAX_REG_CLASSES 4

/**
 * P2P_MAX_REG_CLASS_CHANNELS - Maximum number of channels per regulatory class
 */
#define P2P_MAX_REG_CLASS_CHANNELS 14
...
struct p2p_channels {
	...
	struct p2p_reg_class {
		 ...
		 */
		atbm_uint8 channel[P2P_MAX_REG_CLASS_CHANNELS];
...
struct p2p_go_neg_results {
	 ...
	 */
	atbm_uint8 peer_device_addr[ATBM_ETH_ALEN];

	/**
	 * peer_interface_addr - P2P Interface Address of the peer
	 */
	atbm_uint8 peer_interface_addr[ATBM_ETH_ALEN];
...
struct p2ps_provision {
	 ...
	 */
	atbm_uint8 session_mac[ATBM_ETH_ALEN];

	/**
	 * adv_mac - MAC address of the peer advertised the service
	 */
	atbm_uint8 adv_mac[ATBM_ETH_ALEN];
	 ...
	 */
	atbm_uint8 cpt_priority[P2PS_FEATURE_CAPAB_CPT_MAX + 1];
...
#define P2P_MAX_WPS_VENDOR_EXT 3
...
struct p2p_peer_info {
	 ...
	 */
	atbm_uint8 p2p_device_addr[ATBM_ETH_ALEN];

	/**
	 * pri_dev_type - Primary Device Type
	 */
	atbm_uint8 pri_dev_type[8];
	 ...
	 */
	atbm_uint8 wps_sec_dev_type_list[WPS_SEC_DEV_TYPE_MAX_LEN];

	/**
	 * wps_sec_dev_type_list_len - Length of secondary device type list
	 */
	atbm_size_t wps_sec_dev_type_list_len;
...
struct p2p_config {
	 ...
	 */
	atbm_uint8 channel_forced;

	/**
	 * Regulatory class for own operational channel
	 */
	atbm_uint8 op_reg_class;
...
#define P2P_SEC_DEVICE_TYPES 5
	 ...
	 */
	atbm_uint8 sec_dev_type[P2P_SEC_DEVICE_TYPES][8];

	/**
	 * num_sec_dev_types - Number of sec_dev_type entries
	 */
	atbm_size_t num_sec_dev_types;

	/**
	 * dev_addr - P2P Device Address
	 */
	atbm_uint8 dev_addr[ATBM_ETH_ALEN];
	 ...
	 */
	atbm_uint8 ssid_postfix[SSID_MAX_LEN - 9];

	/**
	 * ssid_postfix_len - Length of the ssid_postfix data
	 */
	atbm_size_t ssid_postfix_len;
	 ...
	 * transmit.
	 */
	int (*send_action)(void *ctx, unsigned int freq, const atbm_uint8 *dst,
	 ...
	 */
	int (*get_noa)(void *ctx, const atbm_uint8 *interface_addr, atbm_uint8 *buf,
		       atbm_size_t buf_len);
	 ...
	 */
	void (*dev_found)(void *ctx, const atbm_uint8 *addr,
	 ...
	 */
	void (*dev_lost)(void *ctx, const atbm_uint8 *dev_addr);
	 ...
	 * call to p2p_connect().
	 */
	void (*go_neg_req_rx)(void *ctx, const atbm_uint8 *src, atbm_uint16 dev_passwd_id,
			      atbm_uint8 go_intent);

	int (*peer_waiting_cb)(void *ctx, const atbm_uint8 *src);
	 ...
	 */
	void (*sd_request)(void *ctx, int freq, const atbm_uint8 *sa, atbm_uint8 dialog_token,
	 ...
	 */
	void (*sd_response)(void *ctx, const atbm_uint8 *sa, atbm_uint16 update_indic,
	 ...
	 */
	void (*prov_disc_req)(void *ctx, const atbm_uint8 *peer, atbm_uint16 config_methods,
			      const atbm_uint8 *dev_addr, const atbm_uint8 *pri_dev_type,
			      const char *dev_name, atbm_uint16 supp_config_methods,
			      atbm_uint8 dev_capab, atbm_uint8 group_capab,
			      const atbm_uint8 *group_id, atbm_size_t group_id_len);
	 ...
	 */
	void (*prov_disc_resp)(void *ctx, const atbm_uint8 *peer, atbm_uint16 config_methods);
	 ...
	 */
	void (*prov_disc_fail)(void *ctx, const atbm_uint8 *peer,
	 ...
	 */
	atbm_uint8 (*invitation_process)(void *ctx, const atbm_uint8 *sa, const atbm_uint8 *bssid,
	 ...
	 */
	void (*invitation_received)(void *ctx, const atbm_uint8 *sa, const atbm_uint8 *bssid,
	 ...
	 */
	void (*invitation_result)(void *ctx, int status, const atbm_uint8 *bssid,
				  const struct p2p_channels *channels,
				  const atbm_uint8 *addr, int freq, int peer_oper_freq);
	 ...
	 * or 0 if not.
	 */
	int (*go_connected)(void *ctx, const atbm_uint8 *dev_addr);
	 ...
	 */
	void (*presence_resp)(void *ctx, const atbm_uint8 *src, atbm_uint8 status,
	 ...
	 */
	int (*get_persistent_group)(void *ctx, const atbm_uint8 *addr, const atbm_uint8 *ssid,
	 ...
	 */
	int (*get_go_info)(void *ctx, atbm_uint8 *intended_addr,
	 ...
	 */
	int (*remove_stale_groups)(void *ctx, const atbm_uint8 *peer, const atbm_uint8 *go,
				   const atbm_uint8 *ssid, atbm_size_t ssid_len);
	 ...
	 */
	void (*p2ps_prov_complete)(void *ctx, atbm_uint8 status, const atbm_uint8 *dev,
				   ...
				   unsigned int freq, const atbm_uint8 *group_ssid,
	 ...
	 */
	atbm_uint8 (*p2ps_group_capability)(void *ctx, atbm_uint8 incoming, atbm_uint8 role,
				    unsigned int *force_freq,
				    unsigned int *pref_freq);
...
int p2p_find(struct p2p_data *p2p, unsigned int timeout,
	     ...
	     atbm_uint8 seek_count, const char **seek_string, int freq);
 ...
 */
int p2p_prov_disc_req(struct p2p_data *p2p, const atbm_uint8 *peer_addr,
		      struct p2ps_provision *p2ps_prov, atbm_uint16 config_methods,
 ...
 * callback.
 */
void * p2p_sd_request(struct p2p_data *p2p, const atbm_uint8 *dst,
		      const struct wpabuf *tlvs);

#ifdef CONFIG_WIFI_DISPLAY
void * p2p_sd_request_wfd(struct p2p_data *p2p, const atbm_uint8 *dst,
			  const struct wpabuf *tlvs);
 ...
 */
void p2p_sd_response(struct p2p_data *p2p, int freq, const atbm_uint8 *dst,
		     atbm_uint8 dialog_token, const struct wpabuf *resp_tlvs);
 ...
 * Returns: 0 on success, -1 on failure
 */
int p2p_invite(struct p2p_data *p2p, const atbm_uint8 *peer, enum p2p_invite_role role,
	       const atbm_uint8 *bssid, const atbm_uint8 *ssid, atbm_size_t ssid_len,
	       unsigned int force_freq, const atbm_uint8 *go_dev_addr,
...
struct p2p_group_config {
	 ...
	 */
	atbm_uint8 interface_addr[ATBM_ETH_ALEN];
 ...
 * Returns: 0 on success, -1 on failure
 */
int p2p_group_notif_assoc(struct p2p_group *group, const atbm_uint8 *addr,
			  const atbm_uint8 *ie, atbm_size_t len);
 ...
 * wpabuf_free().
 */
struct wpabuf * p2p_group_assoc_resp_ie(struct p2p_group *group, atbm_uint8 status);
 ...
 * @addr: Interface address of the P2P client
 */
void p2p_group_notif_disassoc(struct p2p_group *group, const atbm_uint8 *addr);
 ...
 */
int p2p_group_notif_noa(struct p2p_group *group, const atbm_uint8 *noa,
			atbm_size_t noa_len);
 ...
 */
int p2p_group_go_discover(struct p2p_group *group, const atbm_uint8 *dev_id,
			  const atbm_uint8 *searching_dev, int rx_freq);
 ...
 */
const atbm_uint8 * p2p_iterate_group_members(struct p2p_group *group, void **next);
 ...
 * found
 */
const atbm_uint8 * p2p_group_get_client_interface_addr(struct p2p_group *group,
					       const atbm_uint8 *dev_addr);
 ...
 * found
 */
const atbm_uint8 * p2p_group_get_dev_addr(struct p2p_group *group, const atbm_uint8 *addr);
 ...
 */
int p2p_group_is_client_connected(struct p2p_group *group, const atbm_uint8 *dev_addr);
...
struct p2p_nfc_params {
	int sel;
	const atbm_uint8 *wsc_attr;
	atbm_size_t wsc_len;
	const atbm_uint8 *p2p_attr;
	atbm_size_t p2p_len;
	...
	atbm_uint8 oob_dev_pw[WPS_OOB_PUBKEY_HASH_LEN + 2 +
		      WPS_OOB_DEVICE_PASSWORD_LEN];
	atbm_size_t oob_dev_pw_len;
	int go_freq;
	atbm_uint8 go_dev_addr[ATBM_ETH_ALEN];
	atbm_uint8 go_ssid[SSID_MAX_LEN];
	atbm_size_t go_ssid_len;
};
...
int p2p_service_add_asp(struct p2p_data *p2p, int auto_accept, atbm_uint32 adv_id,
			const char *adv_str, atbm_uint8 svc_state,
			atbm_uint16 config_methods, const char *svc_info,
			const atbm_uint8 *cpt_priority);
