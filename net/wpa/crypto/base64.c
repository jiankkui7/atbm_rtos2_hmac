...
/**
 * atbmwifi_base64_encode - Base64 encode
			      ...
			      atbm_size_t *out_len)
{
	...
	out = (unsigned char *)atbm_kmalloc(olen, GFP_KERNEL);
	if (out == NULL)
		return NULL;
...
/**
 * atbmwifi_base64_decode - Base64 decode
			      ...
			      atbm_size_t *out_len)
{
	...
	atbm_size_t i, count, olen;
	int pad = 0;

	atbm_memset(dtable, 0x80, 256);
	...
	olen = count / 4 * 3;
	pos = out = (unsigned char *)atbm_kmalloc(olen, GFP_KERNEL);
	if (out == NULL)
		return NULL;
