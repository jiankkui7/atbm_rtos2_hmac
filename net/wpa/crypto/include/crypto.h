 ...
 */
int md4_vector(atbm_size_t num_elem, const atbm_uint8 *addr[], const atbm_size_t *len, atbm_uint8 *mac);
 ...
 * Returns: 0 on success, -1 on failure
 */
int md5_vector(atbm_size_t num_elem, const atbm_uint8 *addr[], const atbm_size_t *len, atbm_uint8 *mac);


/**
 * atbmwifi_sha1_vector - SHA-1 hash for data vector
 * @num_elem: Number of elements in the data vector
 * @addr: Pointers to the data areas
 * @len: Lengths of the data blocks
 * @mac: Buffer for the hash
 * Returns: 0 on success, -1 on failure
 */
int atbmwifi_sha1_vector(atbm_size_t num_elem, const atbm_uint8 *addr[], const atbm_size_t *len,
		atbm_uint8 *mac);
 ...
 */
int   fips186_2_prf(const atbm_uint8 *seed, atbm_size_t seed_len, atbm_uint8 *x,
			       atbm_size_t xlen);

/**
 * atbmwifi_sha256_vector - SHA256 hash for data vector
 * @num_elem: Number of elements in the data vector
 * @addr: Pointers to the data areas
 * @len: Lengths of the data blocks
 * @mac: Buffer for the hash
 * Returns: 0 on success, -1 on failure
 */
int atbmwifi_sha256_vector(atbm_size_t num_elem, const atbm_uint8 *addr[], const atbm_size_t *len,
		  atbm_uint8 *mac);
 ...
 * @cypher: 8 octets (out)
 */
void des_encrypt(const atbm_uint8 *clear, const atbm_uint8 *key, atbm_uint8 *cypher);

/**
 * atbmwifi_aes_encrypt_init - Initialize AES for encryption
 * @key: Encryption key
 * @len: Key length in bytes (usually 16, i.e., 128 bits)
 * Returns: Pointer to context data or %NULL on failure
 */
void * atbmwifi_aes_encrypt_init(const atbm_uint8 *key, atbm_size_t len);

/**
 * atbmwifi_aes_encrypt - Encrypt one AES block
 * @ctx: Context pointer from atbmwifi_aes_encrypt_init()
 * @plain: Plaintext data to be encrypted (16 bytes)
 * @crypt: Buffer for the encrypted data (16 bytes)
 */
void atbmwifi_aes_encrypt(void *ctx, const atbm_uint8 *plain, atbm_uint8 *crypt);

/**
 * atbmwifi_aes_encrypt_deinit - Deinitialize AES encryption
 * @ctx: Context pointer from atbmwifi_aes_encrypt_init()
 */
void atbmwifi_aes_encrypt_deinit(void *ctx);

/**
 * atbmwifi_aes_decrypt_init - Initialize AES for decryption
 * @key: Decryption key
 * @len: Key length in bytes (usually 16, i.e., 128 bits)
 * Returns: Pointer to context data or %NULL on failure
 */
void * atbmwifi_aes_decrypt_init(const atbm_uint8 *key, atbm_size_t len);

/**
 * atbmwifi_aes_decrypt - Decrypt one AES block
 * @ctx: Context pointer from atbmwifi_aes_encrypt_init()
 * @crypt: Encrypted data (16 bytes)
 * @plain: Buffer for the decrypted data (16 bytes)
 */
void atbmwifi_aes_decrypt(void *ctx, const atbm_uint8 *crypt, atbm_uint8 *plain);

/**
 * atbmwifi_aes_decrypt_deinit - Deinitialize AES decryption
 * @ctx: Context pointer from atbmwifi_aes_encrypt_init()
 */
void atbmwifi_aes_decrypt_deinit(void *ctx);
 ...
 * to implement this.
 */
struct crypto_hash * crypto_hash_init(enum crypto_hash_alg alg, const atbm_uint8 *key,
				      atbm_size_t key_len);
 ...
 * to implement this.
 */
void crypto_hash_update(struct crypto_hash *ctx, const atbm_uint8 *data, atbm_size_t len);
 ...
 * to implement this.
 */
int crypto_hash_finish(struct crypto_hash *ctx, atbm_uint8 *hash, atbm_size_t *len);
...
struct crypto_cipher * crypto_cipher_init(enum crypto_cipher_alg alg,
					  const atbm_uint8 *iv, const atbm_uint8 *key,
					  atbm_size_t key_len);
...
int   crypto_cipher_encrypt(struct crypto_cipher *ctx,
				       const atbm_uint8 *plain, atbm_uint8 *crypt, atbm_size_t len);
...
int   crypto_cipher_decrypt(struct crypto_cipher *ctx,
				       const atbm_uint8 *crypt, atbm_uint8 *plain, atbm_size_t len);
 ...
 * to implement this.
 */
struct crypto_public_key * crypto_public_key_import(const atbm_uint8 *key, atbm_size_t len);

struct crypto_public_key *
crypto_public_key_import_parts(const atbm_uint8 *n, atbm_size_t n_len,
			       const atbm_uint8 *e, atbm_size_t e_len);
 ...
 * to implement this.
 */
struct crypto_private_key * crypto_private_key_import(const atbm_uint8 *key,
						      atbm_size_t len,
						      const char *passwd);
 ...
 * to implement this.
 */
struct crypto_public_key * crypto_public_key_from_cert(const atbm_uint8 *buf,
						       atbm_size_t len);
...
int   crypto_public_key_encrypt_pkcs1_v15(
	struct crypto_public_key *key, const atbm_uint8 *in, atbm_size_t inlen,
...
int   crypto_private_key_decrypt_pkcs1_v15(
	struct crypto_private_key *key, const atbm_uint8 *in, atbm_size_t inlen,
	atbm_uint8 *out, atbm_size_t *outlen);
...
int   crypto_private_key_sign_pkcs1(struct crypto_private_key *key,
					       const atbm_uint8 *in, atbm_size_t inlen,
					       atbm_uint8 *out, atbm_size_t *outlen);
...
int   crypto_public_key_decrypt_pkcs1(
	struct crypto_public_key *key, const atbm_uint8 *crypt, atbm_size_t crypt_len,
	atbm_uint8 *plain, atbm_size_t *plain_len);
...
/**
 * atbmwifi_rc4_skip - XOR RC4 stream to given data with skip-stream-start
 * @key: RC4 key
 ...
 */
int crypto_get_random(void *buf, atbm_size_t len);
