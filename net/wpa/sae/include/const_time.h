...
/* Returns: -1 if a == b; 0 if a != b */
static inline atbm_uint8 const_time_eq_u8(unsigned int a, unsigned int b)
{
	return (atbm_uint8) const_time_eq(a, b);
}
...
static inline unsigned int const_time_eq_bin(const void *a, const void *b,
					     atbm_size_t len)
{
	const atbm_uint8 *aa = a;
	const atbm_uint8 *bb = b;
	atbm_size_t i;
	atbm_uint8 res = 0;
 ...
 * Returns: true_val if mask == -1, false_val if mask == 0
 */
static inline atbm_uint8 const_time_select_u8(atbm_uint8 mask, atbm_uint8 true_val, atbm_uint8 false_val)
{
	return (atbm_uint8) const_time_select(mask, true_val, false_val);
}
 ...
 * Returns: true_val if mask == -1, false_val if mask == 0
 */
static inline atbm_int8 const_time_select_s8(atbm_uint8 mask, atbm_int8 true_val, atbm_int8 false_val)
{
	return (atbm_int8) const_time_select(mask, (unsigned int) true_val,
				      (unsigned int) false_val);
}
 ...
 * is being copied.
 */
static inline void const_time_select_bin(atbm_uint8 mask, const atbm_uint8 *true_val,
					 const atbm_uint8 *false_val, atbm_size_t len,
					 atbm_uint8 *dst)
{
	atbm_size_t i;
...
static inline int const_time_memcmp(const void *a, const void *b, atbm_size_t len)
{
	const atbm_uint8 *aa = a;
	const atbm_uint8 *bb = b;
	int diff, res = 0;
