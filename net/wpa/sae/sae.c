...
int sae_set_group(struct sae_data *sae, int group)
{
	...
	if (!dragonfly_suitable_group(group, 0)) {
		wifi_printk(WIFI_SAE, "SAE: Reject unsuitable group %d", group);
		return -1;
	}
#endif /* CONFIG_TESTING_OPTIONS */

	sae_clear_data(sae);
	tmp = sae->tmp = atbm_kzalloc(sizeof(*tmp), GFP_KERNEL);
	if (tmp == NULL)
		return -1;
	...
	if (tmp->ec) {
		wifi_printk(WIFI_SAE, "SAE: Selecting supported ECC group %d",
			   group);
		sae->group = group;
	...
	if (tmp->dh) {
		wifi_printk(WIFI_SAE, "SAE: Selecting supported FFC group %d",
			   group);
		sae->group = group;
...
void sae_clear_data(struct sae_data *sae)
{
	...
	atbm_memset(sae, 0, sizeof(*sae));
}


static void sae_pwd_seed_key(const atbm_uint8 *addr1, const atbm_uint8 *addr2, atbm_uint8 *key)
{
	wifi_printk(WIFI_SAE, "SAE: PWE derivation - addr1=" MACSTR
		   " addr2=" MACSTR, MAC2STR(addr1), MAC2STR(addr2));
	if (atbm_memcmp(addr1, addr2, ATBM_ETH_ALEN) > 0) {
		atbm_memcpy(key, addr1, ATBM_ETH_ALEN);
		atbm_memcpy(key + ATBM_ETH_ALEN, addr2, ATBM_ETH_ALEN);
	} else {
		atbm_memcpy(key, addr2, ATBM_ETH_ALEN);
		atbm_memcpy(key + ATBM_ETH_ALEN, addr1, ATBM_ETH_ALEN);
	}
}


static int sae_test_pwd_seed_ecc(struct sae_data *sae, const atbm_uint8 *pwd_seed,
				 const atbm_uint8 *prime, const atbm_uint8 *qr, const atbm_uint8 *qnr,
				 atbm_uint8 *pwd_value)
{
	...
	if (atbmwifi_sha256_prf_bits(pwd_seed, SHA256_MAC_LEN, "SAE Hunting and Pecking",
...
static int sae_test_pwd_seed_ffc(struct sae_data *sae, const atbm_uint8 *pwd_seed,
				 struct crypto_bignum *pwe)
{
	atbm_uint8 pwd_value[SAE_MAX_PRIME_LEN];
	atbm_size_t bits = sae->tmp->prime_len * 8;
	atbm_uint8 exp[1];
	struct crypto_bignum *a, *b = NULL;
	int res, is_val;
	atbm_uint8 pwd_value_valid;
...
static int sae_derive_pwe_ecc(struct sae_data *sae, const atbm_uint8 *addr1,
			      const atbm_uint8 *addr2, const atbm_uint8 *password,
			      atbm_size_t password_len, const char *identifier)
{
	atbm_uint8 counter, k;
	atbm_uint8 addrs[2 * ATBM_ETH_ALEN];
	const atbm_uint8 *addr[3];
	atbm_size_t len[3];
	atbm_size_t num_elem;
	atbm_uint8 *dummy_password, *tmp_password;
	int pwd_seed_odd = 0;
	atbm_uint8 prime[SAE_MAX_ECC_PRIME_LEN];
	atbm_size_t prime_len;
	struct crypto_bignum *x = NULL, *qr = NULL, *qnr = NULL;
	atbm_uint8 x_bin[SAE_MAX_ECC_PRIME_LEN];
	atbm_uint8 x_cand_bin[SAE_MAX_ECC_PRIME_LEN];
	atbm_uint8 qr_bin[SAE_MAX_ECC_PRIME_LEN];
	atbm_uint8 qnr_bin[SAE_MAX_ECC_PRIME_LEN];
	int res = -1;
	atbm_uint8 found = 0; /* 0 (false) or 0xff (true) to be used as const_time_*
		       * mask */
	atbm_memset(x_bin, 0, sizeof(x_bin));

	dummy_password = atbm_kmalloc(password_len, GFP_KERNEL);
	tmp_password = atbm_kmalloc(password_len, GFP_KERNEL);
	...
	if (identifier)
		wifi_printk(WIFI_SAE, "SAE: password identifier: %s",
			   identifier);
	...
	num_elem = 1;
	if (identifier) {
		addr[num_elem] = (const atbm_uint8 *) identifier;
	...
	for (counter = 1; counter <= k || !found; counter++) {
		atbm_uint8 pwd_seed[SHA256_MAC_LEN];

		if (counter > 200) {
			/* This should not happen in practice */
			wifi_printk(WIFI_SAE, "SAE: Failed to derive PWE");
			break;
		}

		wifi_printk(WIFI_SAE, "SAE: counter = %03u", counter);
		const_time_select_bin(found, dummy_password, password,
				      password_len, tmp_password);

		if (atbmwifi_hmac_sha256_vector(addrs, sizeof(addrs), num_elem,
				       addr, len, pwd_seed) < 0)
			break;
		...
		atbm_memset(pwd_seed, 0, sizeof(pwd_seed));
		if (res < 0)
			goto fail;
		 ...
		 */
		found |= res * 0xff;
		wifi_printk(WIFI_SAE, "SAE: pwd-seed result %d found=0x%02x\n",
			   res, found);
	}

	if (!found) {
		wifi_printk(WIFI_SAE, "SAE: Could not generate PWE");
		res = -1;
		goto fail;
	}
	...
	if (res < 0) {
		 ...
		 * is a result.
		 */
		wifi_printk(WIFI_SAE, "SAE: Could not solve y");
	}

fail:
	crypto_bignum_deinit(qr, 0);
	crypto_bignum_deinit(qnr, 0);
	atbm_kfree(dummy_password);
	bin_clear_free(tmp_password, password_len);
	crypto_bignum_deinit(x, 1);
	atbm_memset(x_bin, 0, sizeof(x_bin));
	atbm_memset(x_cand_bin, 0, sizeof(x_cand_bin));

	return res;
}


static int sae_derive_pwe_ffc(struct sae_data *sae, const atbm_uint8 *addr1,
			      const atbm_uint8 *addr2, const atbm_uint8 *password,
			      atbm_size_t password_len, const char *identifier)
{
	atbm_uint8 counter, k, sel_counter = 0;
	atbm_uint8 addrs[2 * ATBM_ETH_ALEN];
	const atbm_uint8 *addr[3];
	atbm_size_t len[3];
	atbm_size_t num_elem;
	...
	atbm_uint8 mask;
	struct crypto_bignum *pwe;
	atbm_size_t prime_len = sae->tmp->prime_len * 8;
	atbm_uint8 *pwe_buf;
	 ...
	 * time selection. */
	pwe_buf = atbm_kzalloc(prime_len * 2, GFP_KERNEL);
	...
	for (counter = 1; counter <= k || !found; counter++) {
		...
		wifi_printk(WIFI_SAE, "SAE: counter = %02u\n", counter);
		if (atbmwifi_hmac_sha256_vector(addrs, sizeof(addrs), num_elem,
				       addr, len, pwd_seed) < 0)
			break;
	...
	wifi_printk(WIFI_SAE, "SAE: Use PWE from counter = %02u\n", sel_counter);
					 ...
					 struct crypto_bignum *mask)
{
	...
	if (crypto_ec_point_mul(sae->tmp->ec, sae->tmp->pwe_ecc, mask,
	    ...
	    crypto_ec_point_invert(sae->tmp->ec,
				   sae->tmp->own_commit_element_ecc) < 0) {
		wifi_printk(WIFI_SAE, "SAE: Could not compute commit-element\n");
		return -1;
	}
					 ...
					 struct crypto_bignum *mask)
{
	...
	if (crypto_bignum_exptmod(sae->tmp->pwe_ffc, mask, sae->tmp->prime,
	    ...
	    crypto_bignum_inverse(sae->tmp->own_commit_element_ffc,
				  sae->tmp->prime,
				  sae->tmp->own_commit_element_ffc) < 0) {
		wifi_printk(WIFI_DBG_ERROR, "SAE: Could not compute commit-element\n");
		return -1;
	}
...
int sae_prepare_commit(const atbm_uint8 *addr1, const atbm_uint8 *addr2,
		       const atbm_uint8 *password, atbm_size_t password_len,
...
static int sae_derive_k_ecc(struct sae_data *sae, atbm_uint8 *k)
{
	...
	if (crypto_ec_point_mul(sae->tmp->ec, sae->tmp->pwe_ecc,
		...
		wifi_printk(WIFI_DBG_ERROR, "SAE: Failed to calculate K and k\n");
		goto fail;
	}
...
static int sae_derive_k_ffc(struct sae_data *sae, atbm_uint8 *k)
{
...
static int sae_derive_keys(struct sae_data *sae, const atbm_uint8 *k)
{
	atbm_uint8 null_key[SAE_KEYSEED_KEY_LEN], val[SAE_MAX_PRIME_LEN];
	atbm_uint8 keyseed[SHA256_MAC_LEN];
	atbm_uint8 keys[SAE_KCK_LEN + SAE_PMK_LEN];
	...
	atbm_memset(null_key, 0, sizeof(null_key));
	atbmwifi_hmac_sha256(null_key, sizeof(null_key), k, sae->tmp->prime_len,
		    keyseed);
	...
	if (atbmwifi_sha256_prf(keyseed, sizeof(keyseed), "SAE KCK and PMK",
		       val, sae->tmp->order_len, keys, sizeof(keys)) < 0)
		goto fail;
	atbm_memset(keyseed, 0, sizeof(keyseed));
	atbm_memcpy(sae->tmp->kck, keys, SAE_KCK_LEN);
	atbm_memcpy(sae->pmk, keys + SAE_KCK_LEN, SAE_PMK_LEN);
	atbm_memcpy(sae->pmkid, val, SAE_PMKID_LEN);
	atbm_memset(keys, 0, sizeof(keys));
...
{
	atbm_uint8 k[SAE_MAX_PRIME_LEN];
	if (sae->tmp == NULL ||
		      ...
		      const struct wpabuf *token, const char *identifier)
{
	...
	if (identifier) {
		...
		wpabuf_put_u8(buf, ATBM_WLAN_EID_EXTENSION);
		wpabuf_put_u8(buf, 1 + strlen(identifier));
		wpabuf_put_u8(buf, ATBM_WLAN_EID_EXT_PASSWORD_IDENTIFIER);
		wpabuf_put_str(buf, identifier);
		wifi_printk(WIFI_ALWAYS, "SAE: own Password Identifier: %s",
			   identifier);
	}
}


atbm_uint16 sae_group_allowed(struct sae_data *sae, int *allowed_groups, atbm_uint16 group)
{
	if (allowed_groups) {
		int i;
		...
		if (allowed_groups[i] != group) {
			wifi_printk(WIFI_SAE, "SAE: Proposed group %u not "
				   "enabled in the current configuration",
				   group);
			return ATBM_WLAN_STATUS_FINITE_CYCLIC_GROUP_NOT_SUPPORTED;
		}
	}

	if (sae->state == SAE_COMMITTED && group != sae->group) {
		wifi_printk(WIFI_SAE, "SAE: Do not allow group to be changed");
		return ATBM_WLAN_STATUS_FINITE_CYCLIC_GROUP_NOT_SUPPORTED;
	}

	if (group != sae->group && sae_set_group(sae, group) < 0) {
		wifi_printk(WIFI_SAE, "SAE: Unsupported Finite Cyclic Group %u",
			   group);
		return ATBM_WLAN_STATUS_FINITE_CYCLIC_GROUP_NOT_SUPPORTED;
	}

	if (sae->tmp == NULL) {
		wifi_printk(WIFI_SAE, "SAE: Group information not yet initialized");
		return ATBM_WLAN_STATUS_UNSPECIFIED_FAILURE;
	}

	if (sae->tmp->dh && !allowed_groups) {
		wifi_printk(WIFI_SAE, "SAE: Do not allow FFC group %u without "
...
static int sae_is_password_id_elem(const atbm_uint8 *pos, const atbm_uint8 *end)
{
	return end - pos >= 3 &&
		pos[0] == ATBM_WLAN_EID_EXTENSION &&
		pos[1] >= 1 &&
		end - pos - 2 >= pos[1] &&
		pos[2] == ATBM_WLAN_EID_EXT_PASSWORD_IDENTIFIER;
}


static void sae_parse_commit_token(struct sae_data *sae, const atbm_uint8 **pos,
				   const atbm_uint8 *end, const atbm_uint8 **token,
				   atbm_size_t *token_len)
{
	atbm_size_t scalar_elem_len, tlen;
	const atbm_uint8 *elem;
	...
	if (scalar_elem_len >= (atbm_size_t) (end - *pos))
...
static atbm_uint16 sae_parse_commit_scalar(struct sae_data *sae, const atbm_uint8 **pos,
				   const atbm_uint8 *end)
{
	struct crypto_bignum *peer_scalar;

	if (sae->tmp->prime_len > end - *pos) {
		wifi_printk(WIFI_SAE, "SAE: Not enough data for scalar");
		return ATBM_WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	...
	if (sae->state == SAE_ACCEPTED && sae->peer_commit_scalar &&
		...
		wifi_printk(WIFI_SAE, "SAE: Do not accept re-use of previous "
			   "peer-commit-scalar");
	...
	if (crypto_bignum_is_zero(peer_scalar) ||
		...
		wifi_printk(WIFI_SAE, "SAE: Invalid peer scalar");
		crypto_bignum_deinit(peer_scalar, 0);
		return ATBM_WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
...
static atbm_uint16 sae_parse_commit_element_ecc(struct sae_data *sae, const atbm_uint8 **pos,
					const atbm_uint8 *end)
{
	atbm_uint8 prime[SAE_MAX_ECC_PRIME_LEN];

	if (2 * sae->tmp->prime_len > end - *pos) {
		wifi_printk(WIFI_SAE, "SAE: Not enough data for "
			   "commit-element");
		return ATBM_WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	...
	if (atbm_memcmp(*pos, prime, sae->tmp->prime_len) >= 0 ||
	    atbm_memcmp(*pos + sae->tmp->prime_len, prime,
		      sae->tmp->prime_len) >= 0) {
		wifi_printk(WIFI_SAE, "SAE: Invalid coordinates in peer "
			   "element");
		return ATBM_WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	...
	if (!crypto_ec_point_is_on_curve(sae->tmp->ec,
					 sae->tmp->peer_commit_element_ecc)) {
		wifi_printk(WIFI_SAE, "SAE: Peer element is not on curve");
		return ATBM_WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
...
static atbm_uint16 sae_parse_commit_element_ffc(struct sae_data *sae, const atbm_uint8 **pos,
					const atbm_uint8 *end)
{
	struct crypto_bignum *res, *one;
	const atbm_uint8 one_bin[1] = { 0x01 };
	...
	if (!res || !one ||
		...
		wifi_printk(WIFI_SAE, "SAE: Invalid peer element");
		return ATBM_WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	crypto_bignum_deinit(one, 0);
	...
	if (crypto_bignum_exptmod(sae->tmp->peer_commit_element_ffc,
		...
		wifi_printk(WIFI_SAE, "SAE: Invalid peer element (scalar-op)");
		crypto_bignum_deinit(res, 0);
		return ATBM_WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	crypto_bignum_deinit(res, 0);
...
static atbm_uint16 sae_parse_commit_element(struct sae_data *sae, const atbm_uint8 **pos,
				    const atbm_uint8 *end)
{
	if (sae->tmp->dh)
...
static int sae_parse_password_identifier(struct sae_data *sae,
					 const atbm_uint8 *pos, const atbm_uint8 *end)
{
	...
	if (!sae_is_password_id_elem(pos, end)) {
		...
		atbm_kfree(sae->tmp->pw_id);
		sae->tmp->pw_id = NULL;
		return ATBM_WLAN_STATUS_SUCCESS; /* No Password Identifier */
	}

	if (sae->tmp->pw_id &&
	    (pos[1] - 1 != (int) strlen(sae->tmp->pw_id) ||
	     atbm_memcmp(sae->tmp->pw_id, pos + 3, pos[1] - 1) != 0)) {
		wifi_printk(WIFI_SAE,
	...
	atbm_kfree(sae->tmp->pw_id);
	sae->tmp->pw_id = atbm_kmalloc(pos[1], GFP_KERNEL);
	if (!sae->tmp->pw_id)
		return ATBM_WLAN_STATUS_UNSPECIFIED_FAILURE;
	atbm_memcpy(sae->tmp->pw_id, pos + 3, pos[1] - 1);
...
atbm_uint16 sae_parse_commit(struct sae_data *sae, const atbm_uint8 *data, atbm_size_t len,
		     const atbm_uint8 **token, atbm_size_t *token_len, int *allowed_groups)
{
	const atbm_uint8 *pos = data, *end = data + len;
	atbm_uint16 res;

	/* Check Finite Cyclic Group */
	if (end - pos < 2)
		return ATBM_WLAN_STATUS_UNSPECIFIED_FAILURE;
	res = sae_group_allowed(sae, allowed_groups, ATBM_WPA_GET_LE16(pos));
	...
	if (!sae->tmp->own_commit_scalar ||
		...
		return ATBM_WLAN_STATUS_SUCCESS; /* scalars/elements are different */
...
static void sae_cn_confirm(struct sae_data *sae, const atbm_uint8 *sc,
			   const struct crypto_bignum *scalar1,
			   const atbm_uint8 *element1, atbm_size_t element1_len,
			   const struct crypto_bignum *scalar2,
			   const atbm_uint8 *element2, atbm_size_t element2_len,
			   atbm_uint8 *confirm)
{
	const atbm_uint8 *addr[5];
	atbm_size_t len[5];
	atbm_uint8 scalar_b1[SAE_MAX_PRIME_LEN], scalar_b2[SAE_MAX_PRIME_LEN];
	...
	addr[4] = element2;
	len[4] = element2_len;
	atbmwifi_hmac_sha256_vector(sae->tmp->kck, sizeof(sae->tmp->kck), 5, addr, len,
			   confirm);
}


static void sae_cn_confirm_ecc(struct sae_data *sae, const atbm_uint8 *sc,
			       const struct crypto_bignum *scalar1,
			       ...
			       atbm_uint8 *confirm)
{
	atbm_uint8 element_b1[2 * SAE_MAX_ECC_PRIME_LEN];
	atbm_uint8 element_b2[2 * SAE_MAX_ECC_PRIME_LEN];
...
static void sae_cn_confirm_ffc(struct sae_data *sae, const atbm_uint8 *sc,
			       const struct crypto_bignum *scalar1,
			       ...
			       atbm_uint8 *confirm)
{
	atbm_uint8 element_b1[SAE_MAX_PRIME_LEN];
	atbm_uint8 element_b2[SAE_MAX_PRIME_LEN];
...
int sae_check_confirm(struct sae_data *sae, const atbm_uint8 *data, atbm_size_t len)
{
	atbm_uint8 verifier[SHA256_MAC_LEN];

	if (len < 2 + SHA256_MAC_LEN) {
		wifi_printk(WIFI_SAE, "SAE: Too short confirm message");
		return -1;
	}

	wifi_printk(WIFI_SAE, "SAE: peer-send-confirm %u", ATBM_WPA_GET_LE16(data));

	if (!sae->tmp || !sae->peer_commit_scalar ||
	    !sae->tmp->own_commit_scalar) {
		wifi_printk(WIFI_SAE, "SAE: Temporary data not yet available");
		return -1;
	}
	...
	if (atbm_memcmp(verifier, data + 2, SHA256_MAC_LEN) != 0) {
		wifi_printk(WIFI_SAE, "SAE: Confirm mismatch");
