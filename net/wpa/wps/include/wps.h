...
struct wps_credential {
	atbm_uint8 ssid[32];
	atbm_uint16 ssid_len;
	atbm_uint16 auth_type;
	atbm_uint16 encr_type;
	atbm_uint8 key_idx;
	atbm_uint8 key[64];
	atbm_uint16 key_len;
	atbm_uint16 cred_attr_len;
	atbm_uint8 mac_addr[ATBM_ETH_ALEN];
	const atbm_uint8 *cred_attr;
};
...
struct wps_device_data {
	...
	char *model_number;
	char *serial_number;
	atbm_uint8 pri_dev_type[WPS_DEV_TYPE_LEN];
#define WPS_SEC_DEVICE_TYPES 2
	atbm_uint8 sec_dev_type[WPS_SEC_DEVICE_TYPES][WPS_DEV_TYPE_LEN];
	atbm_uint8 num_sec_dev_types;
	atbm_uint8 rf_bands;
	atbm_uint8 p2p;
	atbm_uint16 config_methods;
	atbm_uint32 os_version;
...
const atbm_uint8 * wps_get_uuid_e(const struct wpabuf *msg);
...
struct wps_registrar_config {
	 ...
	 */
	int (*new_psk_cb)(void *ctx, const atbm_uint8 *mac_addr, const atbm_uint8 *p2p_dev_addr,
			  const atbm_uint8 *psk, atbm_size_t psk_len);
	 ...
	 */
	const atbm_uint8 *extra_cred;

	/**
	 * extra_cred_len: Length of extra_cred in octets
	 */
	atbm_uint16 extra_cred_len;
...
union wps_event_data {
	...
	struct wps_event_fail {
		...
		atbm_uint8 peer_macaddr[ATBM_ETH_ALEN];
	} fail;
...
struct wps_context {
	...
	atbm_uint16 ap;
	/**
	 * ap - wps_success wait deauth
	 */
	atbm_uint16 wpa_success_deauth;
	/**
	 ...
	 */
	atbm_uint8 *ap_settings;

	/**
	 * ap_settings_len - Length of ap_settings in octets
	 */
	atbm_size_t ap_settings_len;
...
//			  const atbm_uint8 *uuid, const atbm_uint8 *pin, atbm_size_t pin_len,
//			  int timeout);
//int wps_registrar_invalidate_pin(struct wps_registrar *reg, const atbm_uint8 *uuid);
int wps_registrar_wps_cancel(struct wps_registrar *reg);
//int wps_registrar_unlock_pin(struct wps_registrar *reg, const atbm_uint8 *uuid);
int wps_registrar_button_pushed(struct wps_registrar *reg,
				const atbm_uint8 *p2p_dev_addr);
void wps_registrar_complete(struct wps_registrar *registrar, const atbm_uint8 *uuid_e,
...
int wps_registrar_add_nfc_pw_token(struct wps_registrar *reg,
				   const atbm_uint8 *pubkey_hash, atbm_uint16 pw_id,
				   const atbm_uint8 *dev_pw, atbm_size_t dev_pw_len,
				   int pk_hash_provided_oob);
int wps_registrar_add_nfc_password_token(struct wps_registrar *reg,
					 const atbm_uint8 *oob_dev_pw,
...
atbm_uint32 wps_generate_pin(void);
...
void wps_er_deinit(struct wps_er *er, atbm_void (*cb)(void *ctx), atbm_void *ctx);
void wps_er_set_sel_reg(struct wps_er *er, int sel_reg, atbm_uint16 dev_passwd_id,
			atbm_uint16 sel_reg_config_methods);
int wps_er_pbc(struct wps_er *er, const atbm_uint8 *uuid, const atbm_uint8 *addr);
const atbm_uint8 * wps_er_get_sta_uuid(struct wps_er *er, const atbm_uint8 *addr);
int wps_er_learn(struct wps_er *er, const atbm_uint8 *uuid, const atbm_uint8 *addr,
		 const atbm_uint8 *pin, atbm_size_t pin_len);
int wps_er_set_config(struct wps_er *er, const atbm_uint8 *uuid, const atbm_uint8 *addr,
		      const struct wps_credential *cred);
int wps_er_config(struct wps_er *er, const atbm_uint8 *uuid, const atbm_uint8 *addr,
		  const atbm_uint8 *pin, atbm_size_t pin_len,
		  const struct wps_credential *cred);
struct wpabuf * wps_er_config_token_from_cred(struct wps_context *wps,
					      struct wps_credential *cred);
struct wpabuf * wps_er_nfc_config_token(struct wps_er *er, const atbm_uint8 *uuid,
					const atbm_uint8 *addr);
struct wpabuf * wps_er_nfc_handover_sel(struct wps_er *er,
					struct wps_context *wps, const atbm_uint8 *uuid,
					const atbm_uint8 *addr, struct wpabuf *pubkey);

int wps_dev_type_str2bin(const char *str, atbm_uint8 dev_type[WPS_DEV_TYPE_LEN]);
char * wps_dev_type_bin2str(const atbm_uint8 dev_type[WPS_DEV_TYPE_LEN], char *buf,
			    atbm_size_t buf_len);
void uuid_gen_mac_addr(const atbm_uint8 *mac_addr, atbm_uint8 *uuid);
atbm_uint16 wps_config_methods_str2bin(const char *str);
struct wpabuf * wps_build_nfc_pw_token(atbm_uint16 dev_pw_id,
...
struct wpabuf * wps_build_nfc_handover_sel(struct wps_context *ctx,
					   struct wpabuf *nfc_dh_pubkey,
					   const atbm_uint8 *bssid, int freq);
...
static inline int wps_validate_beacon_probe_resp(const struct wpabuf *wps_ie,
						 int probe, const atbm_uint8 *addr)
{
	return 0;
}
