...
#include "atbm_hal.h"
#include "wpa_debug.h"
extern char *dup_binstr(const atbm_void *src, atbm_size_t len);
...
{
	return wps_build_rf_bands_attr(msg, dev->rf_bands);
}
 ...
 int wps_build_vendor_ext(struct wps_device_data *dev, struct wpabuf *msg)
{
 ...
 static int wps_process_manufacturer(struct wps_device_data *dev, const atbm_uint8 *str,
				    atbm_size_t str_len)
{
	if (str == NULL) {
 ...
 static int wps_process_model_name(struct wps_device_data *dev, const atbm_uint8 *str,
				  atbm_size_t str_len)
{
	if (str == NULL) {
 ...
 static int wps_process_model_number(struct wps_device_data *dev, const atbm_uint8 *str,
				    atbm_size_t str_len)
{
	if (str == NULL) {
 ...
 static int wps_process_serial_number(struct wps_device_data *dev,
				     const atbm_uint8 *str, atbm_size_t str_len)
{
	if (str == NULL) {
 ...
 static int wps_process_dev_name(struct wps_device_data *dev, const atbm_uint8 *str,
				atbm_size_t str_len)
{
	if (str == NULL) {
					...
					const atbm_uint8 *dev_type)
{
	...
	atbm_memcpy(dev->pri_dev_type, dev_type, WPS_DEV_TYPE_LEN);
			     ...
			     struct wps_parse_attr *attr)
{
 ...
 int wps_process_os_version(struct wps_device_data *dev, const atbm_uint8 *ver)
{
	if (ver == NULL) {
	...
	dev->os_version = ATBM_WPA_GET_BE32(ver);
 ...
 int wps_process_rf_bands(struct wps_device_data *dev, const atbm_uint8 *bands)
{
	if (bands == NULL) {
 ...
 atbm_void wps_device_data_dup(struct wps_device_data *dst,
			 const struct wps_device_data *src)
{
 ...
 atbm_void wps_device_data_free(struct wps_device_data *dev)
{
	if(dev->device_name)
		atbm_kfree(dev->device_name);
