...
#define WPS_WORKAROUNDS
 ...
 static atbm_int32 wps_set_vendor_ext_wfa_subelem(struct wps_parse_attr *attr,
					  atbm_uint8 id, atbm_uint8 len, const atbm_uint8 *pos)
{
 ...
 static atbm_int32 wps_parse_vendor_ext_wfa(struct wps_parse_attr *attr, const atbm_uint8 *pos,
				    atbm_uint16 len)
{
 ...
 static atbm_int32 wps_parse_vendor_ext(struct wps_parse_attr *attr, const atbm_uint8 *pos,
				atbm_uint16 len)
{
#if 1
 ...
 atbm_int32 wps_set_attr(struct wps_parse_attr *attr, atbm_uint16 type,
			const atbm_uint8 *pos, atbm_uint16 len)
{
	...
	case ATTR_OOB_DEVICE_PASSWORD:
		if (len < WPS_OOB_PUBKEY_HASH_LEN + 2 ||
		    ...
		    (len < WPS_OOB_PUBKEY_HASH_LEN + 2 +
		     ...
		     ATBM_WPA_GET_BE16(pos + WPS_OOB_PUBKEY_HASH_LEN) !=
 ...
 atbm_int32 wps_parse_msg(const struct wpabuf *msg, struct wps_parse_attr *attr)
{
	...
	while (pos < end) {
		if (end - pos < 4) {
			wpa_printf(MSG_ALWAYS, "WPS: Invalid message - "
				   "%lu bytes remaining",
